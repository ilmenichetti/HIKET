# =============================================================================
# Q-MODEL: RCPP-OPTIMIZED HYBRID INITIALIZATION (STANDALONE, COMMENTED)
# =============================================================================
# C++ accelerated version for 5-15x speedup over pure R implementation.
# Uses Rcpp to compile the computationally intensive matrix-filling loop.
#
# REQUIREMENTS:
# - Rcpp package installed
# - C++ compiler (g++, clang, or Rtools on Windows)
#
# APPROACH: Same hybrid method as base version but with C++ optimization
# =============================================================================

library(Rcpp)

# =============================================================================
# C++ CODE FOR MATRIX FILLING (PERFORMANCE CRITICAL SECTION)
# =============================================================================
# Compiles C++ functions that replace the double nested R loop
...
// Formula: g(t) = 1 / (1 + sum(alpha))^z
// Simple tissues decompose without fragmentation
...
// Calculate g for branches (with gradual fragmentation)
// Uses average alpha over decomposition period (approximation)
// Three-term formula accounts for colonization dynamics
...
// Calculate g for stems (similar to branches, longer fragmentation time)
double calc_g_stems_cpp(NumericVector alpha_vec, int decomp_time, 
  ...
  // Same formula as branches but with stem parameters
  double term1 = 2.0 * (pow(1.0 + alpha_avg * t, 1.0 - z) - 
...
// This is the performance bottleneck in the R version
// [[Rcpp::export]]
List fill_matrices_cpp(int total_years, 
  ...
  // Initialize output matrices (rows = sim years, cols = input years)
  ...
  // Double loop: for each input year, calculate remaining C at each future year
  // This nested loop is why C++ provides such a large speedup
  for (int input_year = 0; input_year < total_years; input_year++) {
    for (int sim_year = input_year; sim_year < total_years; sim_year++) {
      
      int decomp_time = sim_year - input_year + 1;  // Years since input
      int ib = std::min(decomp_time, tmaxb);  // Branch fragmentation progress
      int is = std::min(decomp_time, tmaxs);  // Stem fragmentation progress
      
      // Extract alpha vectors from input to current year (C++ 0-indexed, R 1-indexed)
      ...
      // Calculate remaining fraction for each tissue type
      ...
      double gu_val = calc_g_simple_cpp(alfan_vec, z);  // Same as needles
      
      // Store remaining carbon (input × remaining fraction)
  ...
  // Return all matrices as named list
  return List::create(
...
# =============================================================================
# HELPER FUNCTIONS (R VERSIONS, USED OUTSIDE MATRIX LOOP)
# =============================================================================
...
# =============================================================================
# MAIN RCPP-OPTIMIZED HYBRID Q-MODEL FUNCTION
# =============================================================================
...
q_model_run_hybrid_rcpp <- function(params = Q_MODEL_DEFAULT_PARAMS, 
  ...
  for(sp in names(species_props)) {
    ...
    # =========================================================================
    # This is where the 5-15x speedup comes from!
    # C++ handles the double nested loop much more efficiently than R
