...
if (!isTRUE(all.equal(.hiket_tighten, 1))) {
  .fr <- grep("^p_", names(YASSO15_SIGMA_PPM), value = TRUE)
  if (length(.fr)) {
    YASSO15_SIGMA_PPM[.fr] <- YASSO15_SIGMA_PPM[.fr] * .hiket_tighten
    message(sprintf("[PRIOR TIGHTENING] Yasso15 fraction SDs x %.3f (%d fractions)",
                    .hiket_tighten, length(.fr)))
  } else {
    message("[PRIOR TIGHTENING] Yasso15 has no transfer fractions -- no effect")
  }
}
