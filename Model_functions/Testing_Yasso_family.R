...
PA <- numeric(44)
PA[1]  <- -YASSO07_DEFAULT_PARAMS["alpha_A"]   # negated: function does alfa = -PA[1]
...
PA[35] <- -YASSO07_DEFAULT_PARAMS["alpha_H"]   # negated: function does alfa = -PA[35]
...
# =============================================================================
# Yasso15 official Fortran reference implementation
# =============================================================================

# Compile once:
# system("R CMD SHLIB ./Model_functions/Decomposition_functions/Yasso_original/y15_subroutine.f90")
#dyn.load("./Model_functions/Decomposition_functions/Yasso_original/y15_subroutine.so")
dyn.load("./Model_functions/Decomposition_functions/Yasso_original/y15_ref.so")

# Build 35-element parameter vector matching mod5c theta ordering
p15 <- YASSO15_DEFAULT_PARAMS  # already 35 elements in our ordering
...
TH[15] <- p15["p_WN"]
TH[16] <- p15["p_EN"]
TH[17] <- p15["w1"]       # leaching weights (ignored by mod5c but must be present)
TH[18] <- p15["w2"]
TH[19] <- p15["w3"]
TH[20] <- p15["w4"]
TH[21] <- p15["w5"]
TH[22] <- p15["beta1"]    # AWE temperature
TH[23] <- p15["beta2"]
TH[24] <- p15["betaN1"]   # N temperature
TH[25] <- p15["betaN2"]
TH[26] <- p15["betaH1"]   # H temperature
TH[27] <- p15["betaH2"]
TH[28] <- p15["gamma"]    # AWE precip
TH[29] <- p15["gammaN"]
TH[30] <- p15["gammaH"]
TH[31] <- p15["p_H"]      # humus formation (note: p_H before alpha_H)
TH[32] <- p15["alpha_H"]
TH[33] <- p15["delta1"]
TH[34] <- p15["delta2"]
TH[35] <- p15["r"]

# Helper: call mod5c for one cohort, one timestep
.mod5c <- function(climate, init, b, d, leac = 0.0, steady = FALSE) {
  result <- .Fortran("mod5c_r",
                     theta   = as.single(TH),
                     time    = as.single(1.0),
                     climate = as.single(climate),
                     init    = as.single(init),
                     b       = as.single(b),
                     d       = as.single(d),
                     leac    = as.single(leac),
                     xt      = single(5),
                     steady  = as.logical(steady)
  )
  as.numeric(result$xt)
}

result_Yasso15_ref <- do.call(rbind, lapply(plots, function(pid) {
  
  clim   <- Yasso07_climate[Yasso07_climate$plot_id == pid, ]
  inputs <- Yasso07_inputs[Yasso07_inputs$plot_id   == pid, ]
  years  <- clim$year
  
  # Stage 2: Steady state using mod5c with steadystate_pred = TRUE
  # Run each cohort separately with mean climate and mean inputs
  mean_climate <- c(mean(clim$temp_mean), mean(clim$precip), mean(clim$temp_amplitude))
  
  C_nwl <- .mod5c(mean_climate,
                  init    = rep(0, 5),
                  b       = c(colMeans(inputs[, c("nwl_A","nwl_W","nwl_E","nwl_N")]), 0),
                  d       = 0, steady = TRUE)
  C_fwl <- .mod5c(mean_climate,
                  init    = rep(0, 5),
                  b       = c(colMeans(inputs[, c("fwl_A","fwl_W","fwl_E","fwl_N")]), 0),
                  d       = 2, steady = TRUE)
  C_cwl <- .mod5c(mean_climate,
                  init    = rep(0, 5),
                  b       = c(colMeans(inputs[, c("cwl_A","cwl_W","cwl_E","cwl_N")]), 0),
                  d       = 15, steady = TRUE)
  
  # Stage 3: Transient simulation year by year
  out <- do.call(rbind, lapply(seq_along(years), function(i) {
    climate_i <- c(clim$temp_mean[i], clim$precip[i], clim$temp_amplitude[i])
    
    C_nwl <<- .mod5c(climate_i,
                     init = C_nwl,
                     b    = c(as.numeric(inputs[i, c("nwl_A","nwl_W","nwl_E","nwl_N")]), 0),
                     d    = 0)
    C_fwl <<- .mod5c(climate_i,
                     init = C_fwl,
                     b    = c(as.numeric(inputs[i, c("fwl_A","fwl_W","fwl_E","fwl_N")]), 0),
                     d    = 2)
    C_cwl <<- .mod5c(climate_i,
                     init = C_cwl,
                     b    = c(as.numeric(inputs[i, c("cwl_A","cwl_W","cwl_E","cwl_N")]), 0),
                     d    = 15)
    
    data.frame(year = years[i], total_soc = sum(C_nwl + C_fwl + C_cwl))
  }))
...
model_colors <- c(
  ...
  "Yasso15 (Fortran)"  = "#d6604d",
  "Yasso15 (ref F90)"  = "#f4a582",
  "Yasso20"            = "#1a9641"
)
