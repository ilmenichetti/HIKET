...
The rule this respects is not ``a campaign-specific bias is forbidden'' --- that
phrasing is both too strong and inconsistent with our own practice, since treatment~C
\emph{is} a campaign-specific offset, applied to 1985 alone and sourced from the
workbook's layer coding. The operating principle is narrower:

\begin{center}
\emph{A campaign offset may be prescribed from external protocol evidence.\\
It may not be estimated from the SOC data it would explain.}
\end{center}

\noindent
which is the same principle that governs every $\tau$ here (residuals may
\emph{check} a design value, never \emph{set} one) and the same one that withdrew~C5.
A \emph{fitted} $\delta$ violates it: aliased with the trend, it returned
$\delta = -0.137$ and the conclusion that no accumulation ever happened.

The marginalised version cannot choose an offset to improve its own fit --- there is
no quantity to choose. It applies a fixed, pre-registered amount of scepticism to
campaign-level contrasts. \textbf{It does cost precision in the trend, by
construction:} at $\tau_C = 0.03$ the 2006--2024 rate is already down to
$0.70\sigma$. What it cannot do is manufacture a \emph{conclusion} about the trend's
sign. A fitted $\delta$ hands you a finding; a marginalised $u^{C}$ hands you wider
error bars.

\begin{warnbox}[Two things ``adds no parameter'' does not mean]
It is not free of consequence for $\theta$. A non-diagonal $\Sigma$ changes the
generalised-least-squares weights, exactly the reweighting already documented for a
campaign-specific $\sigma$ (1985's weight $32\% \to 14\%$). And it absorbs
\emph{information} even though it absorbs no \emph{signal} --- which is the point of
the term, not a defect of it.
\end{warnbox}

There remains a third tier we do not use: $\delta_{1985}$ with an \emph{informative}
prior, centre and width sourced from the protocol audit. That is legitimate in exactly
the way treatment~C was. The reason not to use it today is that the surviving
1985-specific mechanisms (LOI-derived mineral C\%, relocated subplots, the
depth-inverted subsoil gain) have no \emph{signed} magnitude attached, so we could
write a width but not a centre. A symmetric $\tau_C$ is what one writes when the size
is known and the direction is not --- which is our actual state of knowledge.
...
\section{Open thread, to resume}

\begin{warnbox}[Unfinished as of 18 August 2026 --- do not act on this section yet]
The $\sigma_{\text{init}}$ benchmark of 0.90 was traced to
$(1400/1775)^{0.43}$: NFI growing stock in 1917 (extrapolated flat from the 1921--24
inventory) over 1985, raised to a litter--volume elasticity fitted on our own litter
record. It is sound as a statement about \emph{litter flux} and over-strong as one
about \emph{soil carbon stock}, because $\sigma_{\text{init}}$ additionally
equilibrates the 1917 soil at that flux, and the growing-stock record says nothing
about whether 1917 soils had caught up with their inputs.

Two consequences. First, $\sigma_{\text{init}} = 0.90$ is unreachable for the Yasso
models under any error model: it implies 1917 stocks of 92--98\,tC\,ha$^{-1}$, above
their own modelled 1985 values of 57--63. Second, a 1917 stock of
40--45\,tC\,ha$^{-1}$ --- about 65--70\% of the 1985 level, a soil still recovering
from the exploitation era --- is physically reasonable and requires only
$\sigma_{\text{init}} \approx 0.37$--$0.44$.

Proposed but \textbf{not agreed}: judge the 1917 state on the stock and on the
implied pre-run rate rather than on $\sigma_{\text{init}}$ reaching a number. The
rate check is independent of the equilibrium confound. \textbf{Corrected 2026-08-19:}
measured properly (\texttt{doublechecks/init\_state\_plausibility.R}) the pre-run demands
$+0.148$ to $+0.242$\,tC\,ha$^{-1}$\,yr$^{-1}$, i.e.\ \emph{below} the observed modern
$+0.259$ --- so that criterion is satisfied in all six, and the earlier
``$+0.37$ to $+0.40$'' paired the 1917 stock with a 1985 endpoint the initialiser never
reaches. The binding concern is the 1917 \emph{stock} alone: $30$--$40$\,tC\,ha$^{-1}$,
against an observed 1985 mean of $66$ and a documented maximum depletion from historical
over-exploitation of $\sim$20--25\%.

Note also that this run is \emph{not} expected to relieve $\sigma_{\text{init}}$:
$u^{P}$ and $u^{R}$ cancel on within-plot differencing, and $\tau_C$ leaves the trend
constraint about 12\% tighter than the 1985 inflation it replaces. \emph{That last
figure answers a different question from the one usually asked}: relative to the
\emph{other campaigns}, $\tau_C = 0.06$ does downweight 1985 substantially --- its
level carries $2.4\times$ less weight than a 2006 or 2024 level, and raising it to
$0.09$ cuts that to a fifth while the 35-year signal falls only $1.92 \to 1.35\sigma$
(\texttt{doublechecks/campaign\_tau\_bracket.R}). The scheme is simultaneously more
sceptical of 1985 \emph{and} tighter overall, because the total is split rather than
added. If
$\sigma_{\text{init}}$ remains low, revisiting $\tau_C$ is legitimate \emph{only}
on the evidence of the post-run campaign residual spreads, never on the
$\sigma_{\text{init}}$ value itself.
\end{warnbox}
