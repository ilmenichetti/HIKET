...
% Provisional-result box (amber): a figure or number we intend to redo before submission.
\definecolor{provbg}{HTML}{FDF3E3}\definecolor{provrule}{HTML}{C08A2E}
\newtcolorbox{provisional}{colback=provbg, colframe=provrule, boxrule=0pt, leftrule=3pt,
  arc=1pt, left=8pt, right=8pt, top=5pt, bottom=5pt, fontupper=\small,
  before skip=8pt, after skip=8pt}
...
% =====================================================================
% RETIRED 2026-09-02. Kept as the record of the story's DESIGN, not as a
% current description of the paper. Superseded by manuscript/HIKET_main_manuscript.tex,
% which now carries a written Results section and repaired Discussion.
% Do not update this file; do not quote numbers from it.
% =====================================================================
\begin{center}
\fbox{\begin{minipage}{0.93\textwidth}
\small
\textbf{RETIRED --- 2026-09-02.} This note planned the paper; the paper has since
overtaken it. It is kept for the record of the narrative design (the OCAR arc, the
three threads, the scope-bounding of the complexity claim) and for the round-1
coauthor comments it was written against. \textbf{It is no longer maintained, and its
numbers are wrong.} Specifically: the ``$26$~tC\,ha$^{-1}$ above the observed 1985
stock'' offset, the campaign medians $59.4/66.7/67.4$, the reading notes about a
pending recalibration, and every figure reference belong to superseded data and
calibrations. The current statement of the story, with numbers keyed to
RUN\_IDs \texttt{20260831\_1624*}, is \texttt{HIKET\_main\_manuscript.tex}.
\end{minipage}}
\end{center}
...
The same geometry governs the quantity the reader will care about most: our mean residence times come
out well below the published parameterisations. Fig.~\ref{fig:ridge} shows why, and how much of it is
the data's doing. In the Yasso family MRT and $\sinput$ trade off along a clear ridge
($r=-0.79/-0.73/-0.57$): the observed stock pins their \emph{product}, so the split between ``slow
turnover, modest inputs'' and ``fast turnover, inflated inputs'' has to be decided by something other
than the stock. Notably, the \emph{simple} models show no such ridge ($r\approx-0.26/-0.31$), so this
is a property of the Yasso structure, not of the calibration in general.

The cost of moving to the published residence time is then wildly uneven --- so much so that
\textbf{``our MRT is too short'' is not one finding but three different ones}. Yasso20's posterior
straddles its published value (18\,\% of draws exceed it) and sitting there costs $1.6$
log-likelihood units, which is nothing. Yasso15 reaches its published value only in the far tail
(0.011\,\% of draws) at a cost of $14.7$. \textbf{Yasso07 never reaches its published value at all:}
225\,015 draws span $10.4$--$25.4$\,yr against a published $33.5$, so the posterior stops eight years
short and the figure can put no price on the gap --- only a profile likelihood could.

\emph{Which published value.} Those costs are measured against the published \emph{point}
parameterisation (33.5 / 30.4 / 19.0\,yr). Yasso15 and Yasso20 also publish posterior \emph{samples},
and because residence time is a nonlinear many-to-one function of the parameters, the median residence
time of that sample is not the residence time of the median parameters --- for Yasso20 it is 25.0
rather than 19.0\,yr, which raises its cost from 1.6 to 10.0. The qualitative ordering is unchanged, but
the two comparators must not be mixed, and the figure uses the point throughout.
...
(intrinsic MRT, $\sinput$) for each Yasso model. \textbf{Colours are the likelihood alone --- the fit
to the data, priors excluded --- blue is the best fit, through green and yellow to red at the worst. The white contour lines are
the posterior --- likelihood \emph{and} priors together.} Colour is the log-likelihood deficit from
the best fit found, clamped at 25 units. Dashed line: published MRT; white dot: posterior median.
Regions the sampler visited fewer than five times are left blank. \emph{Bottom:} first-order variance
indices of MRT. The colour peak lies up and to the left of the contours in all three panels: the data alone want an
\emph{even shorter} residence time, and it is the prior that pulls the answer back toward the
published value.}\label{fig:ridge}
\end{figure}
...
\emph{Three honest limits.} (i) The colours are prior-free in \emph{value} but not in \emph{coverage}:
each cell shows the best of the draws the sampler happened to visit, and the sampler was steered by
the priors. It is therefore a lower bound on the true best fit, and it is weakest exactly where draws
are thin --- the band around the published value holds 121\,141 draws for Yasso20 but only 741 for
Yasso15 and \emph{none at all} for Yasso07. A blank cell means \emph{not visited}, never \emph{cannot
be fitted}, and Yasso07's blank is precisely why a profile likelihood is needed. (ii) The colour surface is estimated on a coarse grid --- one whose cells hold enough draws to
support a maximum --- and then bilinearly interpolated for display, so it looks continuous without
claiming resolution the draws cannot support. (iii) Colour is the log-likelihood \emph{deficit} from
the best fit found, clamped at 25 units: a deficit past that is decisively rejected, so resolving 30
from 200 would convey nothing and would flatten the few units of structure along the ridge that
actually matter.
\end{methodnote}

\begin{provisional}
\textbf{To be redone before submission.} Replace the sampled colour map with a true \emph{profile
likelihood} --- an optimisation over the remaining parameters at each grid node --- which removes the
prior from the coverage as well as from the value, prices the region beyond the posterior's reach,
and is smooth enough to interpolate. Roughly 7\,h per model on a $20\times20$ grid; \textbf{Yasso07
first}, since it is the model for which no sampled estimate exists at all. Expect the figure to change again
once the $\sinput$ prior is tightened.
\end{provisional}

\subsection{Why the climate response owns Yasso07's gap and cannot own the others'}
The lower row of Fig.~\ref{fig:ridge} shows that residence time is made uncertain by different things
in different models: climate dominates in Yasso07, transfer fractions in Yasso15, and the two share
Yasso20. That asymmetry looks at first like an artefact of the priors. It is structural.
...
to $1.822$ ($\times2.13$) at the Finnish mean climate, and that alone predicts a residence time of
$15.7$\,yr against an actual $15.2$ --- the entire gap, with nothing left over. The model has nowhere
else to put a climate misfit. Yasso15 and Yasso20 carry \emph{three} pool-specific modifiers
($\xi_{\mathrm{AWE}},\xi_{\mathrm{N}},\xi_{\mathrm{H}}$); residence time is governed by the slow
humus pool, which has its own $\beta_{H1}$, and that modifier moved only $\times1.04$ and $\times1.06$.
In Yasso20 the components even move in opposite directions ($\xi_{\mathrm{AWE}}\times0.76$ against
$\xi_{\mathrm{N}}\times1.16$) and largely cancel. The leverage Yasso07 hands to climate is
structurally unavailable to its successors.

Two consequences. First, Yasso07's \emph{published} $\xi$ is $0.855<1$: its published parameterisation
says Finnish conditions decompose \emph{more slowly} than its reference, where Yasso15 and Yasso20
already say faster ($1.16$--$1.66$). Yasso07 starts furthest from what the Finnish data want and is the
only variant able to travel the whole distance in one parameter. Second, calibration \textbf{inverts
the family ordering}: published $33.5/30.4/19.0$\,yr (Yasso07 slowest) becomes $15.2/22.1/17.5$
(Yasso07 fastest). Reproduce with \texttt{doublechecks/xi\_published\_vs\_ours.R}.
...
\begin{provisional}
\textbf{Priority future direction (ahead of the stratification test): calibrate the ensemble twice,
under a restrictive and a permissive input assumption, and compare the consequences.}

\emph{Why it is the right experiment.} The likelihood cannot separate high-input/short-MRT from
low-input/long-MRT --- the product $\mathrm{MRT}\times\sinput$ is conserved to 4\,\% and the ridge
correlation reaches $-0.79$ to $-0.92$ across the ensemble. What is easy to miss is that the
\emph{equilibrium} response is degenerate too: at steady state $C = J/(k\xi)$, so a warming that
multiplies $\xi$ by $f$ gives $C' = C/f$ whatever the split, and a proportional change in inputs gives
$\Delta C/C = \Delta J/J$ either way. \textbf{The split is invisible in fit and invisible at
equilibrium --- but not in timing.} MRT \emph{is} the time constant: after 50 years a step change is
$96\,\%$ realised at $\mathrm{MRT}=15$ but only $78\,\%$ at $\mathrm{MRT}=33$. Saturation is a
statement about timing, so the one thing the data cannot constrain is precisely the thing the policy
question turns on. That reframes the equifinality: it is not a nuisance beside the result, it
\emph{is} the uncertainty in the result.

\emph{The primary question, stated precisely.} \textbf{Calibrate the same model under two input
assumptions, obtaining two different MRT posteriors --- by how much does the resulting climate
sensitivity differ?} This is a \emph{within-model, between-calibration} comparison, and it is not
answered by the between-model spread of a single calibration, nor by slicing one posterior along the
ridge: a genuine re-calibration lets every other parameter readjust, which a slice cannot capture.
The only partial signal available now is the ridge correlation between $\log\sinput$ and the
equilibrium climate response, which is $-0.58$ for Yasso07 but only $-0.10$/$-0.16$ for Yasso15/20
--- suggesting the effect would be large where a single $\xi$ ties MRT to climate sensitivity and
small where three pool-specific modifiers decouple them. That is a hypothesis to test, not a result.

\emph{Design.} Two full ensemble calibrations differing \textbf{only} in the \texttt{flux\_pair}
input window --- restrictive (Nordic NPP maximum, $\sinput \le 1.84$) versus permissive (the
present window, which does not bind). Then compare, using the posterior draws from each:
\begin{enumerate}\itemsep2pt
\item \textbf{Fitness} --- the two RMSE distributions from the MCMC samples, not point estimates. If
they overlap, the restrictive assumption costs nothing in fit and the permissive one is unjustified.
\item \textbf{Climate reactivity} --- forward runs sweeping temperature, propagating the posteriors
rather than a median, so the reactivity itself carries the calibration uncertainty.
\item \textbf{Sink or source} --- the sign and magnitude of the contemporary stock change.
\item \textbf{Future extrapolation} --- trajectories to 2100, where the timing difference compounds.
\end{enumerate}

\emph{What would make it publishable either way.} If the two calibrations fit equally well but
project differently, the ensemble's forecast spread is shown to be set by an assumption the data
cannot adjudicate --- a stronger and more honest claim than any single tuned answer. If they project
alike, the input assumption is shown not to matter and the projections are correspondingly more
robust. Note also that pool-specific $\xi$ breaks the symmetry: Yasso07's single modifier makes
$\mathrm{MRT}=\mathrm{MRT}_{\mathrm{ref}}/\xi$ exactly degenerate, while Yasso15/20 respond according
to \emph{which} pools hold the carbon --- so the structural thread reappears in the projections.

\emph{And it points at the data that would settle it.} The degeneracy is a property of the
observation design, not of nature: two SOC observations per plot over 39 years carry almost no
information about a time constant. Radiocarbon constrains turnover directly and independently of
input magnitude; so would higher-frequency repeated measurement, warming manipulations, or
incubations. Stating that turns a limitation into a recommendation.
\end{provisional}
