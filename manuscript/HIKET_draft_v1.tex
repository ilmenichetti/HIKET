...
The same groups
%LORENZO: which are "the same groups"? Qualify, and be in general more specific in the phrasing even at the cost of some redundancy. Implicit objects are usually not good in scientific writing.
...
fragments that occupy part of the soil volume. 
%LORENZO: how? The bulk density and stoniness correction requires data. Which data are we using for that, and from where do they come from?
...
Litter input is taken from a product built from the national forest inventory 
%LORENZO: which product exactly? If it is a source it should be explicit.
and
...
separated into the four chemical fractions used by the Yasso models.
%LORENZO: say in just one phrase how these fractions and separations were achieved, saying if one wantsh more details they are in the reference
 The product
...
particle size, and two auxiliary parameters described below. 
...
Of the two, the rates are the more fundamental to the original model definitions, because the rate of a pool is what
defines that pool chemically.
A compartmental model pool is not a physically separable material 
%LORENZO: this would be controversial, since Yasso INITIALLY was chasing the idea of chemically defined pools. But id did not reallyu work well. Similarly to RothC and the Zimmermann fractionation. We need a literature digression here, extensive but gentle enough towards the Yasso team!
but a position
on a continuum spectrum of decomposability
%LORENZO: here maybe a hook to Bosatta & Ågren literature and their continuous quality theory, just short, not a phrase
, and the rate is what fixes that position, since the
rate sets the mean transit time of carbon through the pool. Changing a rate therefore
fundamentally redefines the pool. The transfer fractions, instead,
describe how carbon moves between pools that have already been defined. We therefore keep
the rates as the published parameterisation set them and calibrate the fractions.
%LORENZO: so the rates are taken as exact values, right? Not tight priors, but exact? This is important to state precisely, if so.
...
First, only the
tree-derived terms are used, meaning tree litter together with harvest residues and
...
Second, the national
totals are converted on a per-hectare basis using the upland forest area implied by the
figures also in \citet{Liski2006}, because the area of forest on upland soils grew over the period and a national
total would otherwise attribute that growth to the soil of individual plots. 
%LORENZO: so dowe have constant or variable areas? Variable, right? Liski reports that variaiton? Wich how much detail?
...
%LORENZO: specify briefly also here what do we take to get the level, pointing to the section relative to it below


Using a reconstructed input rather than just growing stock matters, 
and the difference
...
%LORENZO this paragraph above seems a bit useless, using fluxes instead of biomass is pretty standard practice, no need to defend it

The litter input of 1917 is not known, so we estimated as part of the Bayesian framework. One parameter, which we
...
%LORENZO: now this is one of the key points of our data treatemtn and I would give it more focus.
I would start introducing it, with its advantages. Litter input is very uncertain (and you can for sure find references, Aleksi wrote about it before with Yasso and there is for sure more) in any case, not only initialization, so we solve this problem with sigma_input first, leveraging on the Bayesian framework to implement that ucnertainty in our estimates (already a pretty cool thing).
Then we extend it to the initialization, which even if we pushed it far in time and uncertainty there impacts less than if it was at time zero of the simulation still has an impact. And we again leverage on the Bayesian framework. We aim at estimating accurately the whole uncertainty, including the uncertainty of the inputs.
Might be that we can say that this is rarely done, and this would need to be motivated and well referenced from literature. Would be a nice statement to make, stressing about the uniqueness or anyway advancement of this approach, but would need to be very robust and grounded in literature.
...
%LORENZO: here, this is good. I would frontload this a bit more, higher above, as it is central. Things are already expressed, but focus is a bit off from what I think is the most central part of our data treatment (the two sigmas are pretty big thing)
...
%LORENZO: THis above is controversial, because it is like saying that we will not be able to interprete the comparison. We cannot give it the same amount of information, and we are not doing so. I would frame it as a difficulty, not as a yes/no. The more the models are given different information, and different assumptions, and different degrees of freedom, the less informative the comparison will be. Then we have a series of "solutions" to minimize these problems finding a hopefully best compromise.

In our calibration, the Yasso models are not calibrated for their decomposition rates: the rates are assumed from the published parameterisation as part of the model definition, and what Yasso calibrates are the
transfer fractions between pools, the climate response, the effect of litter
particle size, and the two auxiliary parameters. 
%LORENZO: define the auxiliary parameters, it is not clear like this implicit

The three benchmark simpler models were
therefore given a treatment as similar as possible. Their decomposition rates are partially anchored
...
Pinning the slow rate to informative priors rather than fixing is a compromise, needed because  the sources of external information for Yasso and for the three benchmark models are not of equal quality for
this application. 
The rates that Yasso brings from its own published calibration were
...
cropland would impose kinetics that are likely off for forest soils due to ecological differences between the systems.
%LORENZO: is there some literature to ground this statement, about differences in overall kinetics between these ecosystems? If so add.

Fitting those rates freely, instead, would give the benchmark models a freedom that Yasso does not
have, reducing the value of the comparison. Allowing the data to move the
slow rate (through calibration), but only within a narrow prior around the published value, keeps the two
families as close as possible in freedom while still allowing for some plasticity to adapt to the ecosystem we are modelling. 
This setup is not perfectly symmetric, and the width of the calibrated rate posteriors is reported as a
diagnostic of how far the arable value is from a forest soil. The
...
Yasso's transfer fractions. 

After the anchoring, the free parameters common to SP1, TP2 and
TP3 are identical, and increasing model complexity adds only free partitioning
between pools.
%LORENZO "After the anchoring, the free parameters common to SP1, TP2 and
%TP3 are identical, and increasing model complexity adds only free partitioning
%between pools." pretty weird statement. Free parameters are identical even if structure is more complex? This cannot be. Sounds contradictory statement. Also might not be needed.
...
calibration. 
%LORENZO: this is partially contradicting above, because it is not valid univerally for all models. It is true for sure for Yasso, but is it also absolutely true for the three benchmark models?
Prior widths for the climate and particle-size parameters were read
from the published sources of each model. 
%LORENZO: do we have the same for all models? Particle-size parameters is the litter size in Yasso (don't say particle, sounds like clay or texture), but the other three do not have it.
The twelve transfer fractions of every
model carry the same weakly informative prior on the logit scale. 
%LORENZO: "the same weakly informative prior" seems a self righteous statement. Maybe don't say it even, but as it is sounds wrong. If you want to make the comparison with the phrase below it makes sense but cannot be say like that, what is "the same"? How do you measure it? We just tried oru best to make it as homogenous as possible, seems to me.
...
All six models advance their pools by the analytical solution of the linear system over the
time step,
%LORENZO:"over the time step" is obscure. I thought time was not considered when collapsing the model to the steady state version
 rather than by taking an explicit step forward in time.
%LORENZO: "taking an explicit step forward in time" is obscure, if you mean running them with constant inputs until convergence of some kind

 Each model's pools
form a system in which the amount leaving a pool is proportional to the amount in it,
and such a system has a known solution over any interval, 
%LORENZO: interval of what?
given by the exponential of
...
cascade has a triangular structure whose eigenvalues are known. 
%LORENZO: this is a bit technical, last phrase above. Discuss.
...
that reproduces the exponential to near the precision of the arithmetic. 
%LORENZO is the algorithm really called "twenty-term Taylor series"? Are there some refs to the specific algorithm used (library)? If it's just standard FORTRAN or you developed skip but discuss

None of the
six advances the state by an explicit step of the derivative.
%LORENZO: discuss. This is not understandable by someone with not enough math (like me), needs to be expolained more clearly. We do it together in a discussion, propose alternative and we develop a good one.
...
carbon entering it. 
%LORENZO: so maybe the equation above is not needed for this manuscript? A bit of description should be there, just the equation seems unnecessary.
In practice we therefore give each model a litter input of exactly
...
state (and on the litter input multiplier). The first quantity depends only on the structure of the model and it is what is reported in this paper.
%LORENZO: discuss this with me, sounds confused: the sigmas do not seem relevant here, the key seem to be that our estimate is just not dependent on inputs since we have unit inputs (on top of that there is also the starting state, ours being steady state  does not depend on that). If I am correct as I think this should be written much more plainly, similar to what I stated here.
...
fixed at $0.800$ on the logarithmic scale and is not estimated. 
%LORENZO: how did we come to that assumption, 0.8? Are there some rationales?
The spread of the
model residuals measured on that scale is $0.71$ to $0.74$ across the six models,
%LORENZO: this seems badly circular. Those residuals are estimated after a calibration? If so, it is circular. Discuss.
...
understates how much the observations have in common. 
%LORENZO: everything after "because" above is not right. Might be, but it is not logically connected with the rest. DIscuss options.
...
protocol of that campaign did differently. 
%LORENZO: so they correlate both over time (first, third) and space (second, third to some extent)
The likelihood therefore includes three
...
\]
%LORENZO: not understandiung what these offsets are, if they are not estimated. They seem error terms. Saying "they are not estimated" seems they are prescribed values, which might be the case and might be fine, but these are still error terms, right? We just consider them as known error/uncertainty.
None of the three offsets is estimated. Each offset is integrated out,
%LORENZO: "integrated out" what does it mean in practice? how/when? Discuss.
 so that the
offsets never take a value and leave only their effect on the covariance of the
observations. The standard deviation of each of the three offsets 
%LORENZO: wait, so these offsets have a prior probability distribution? What is set, their mean? How? Discuss.
is fixed at a value
...
latitude bands containing equal numbers of plots. 
%LORENZO: so these latitude bands have been defined in order to contain the same number of plots? "latitude" here is defining that they are horizontally bound, by latitudes, but their definition is then based on the criterion of containing the same number of plots only? Sounds right, just to be sure because might be not totally clear now.
The standard deviation of the plot
...
the two later campaigns.
%LORENZO: are there rationals? Fine if there aren't, but should be stated openly in that case or the reader is left wondering.
 The independent error of a single observation then takes the
remainder of the total, $0.685$.
%LORENZO: which total? Not clear how 0.685 is reached as number.

The four standard deviations combine in quadrature to the fixed total of $0.800$, 
%LORENZO: "combine in quadrature" totally not understandable, discuss. Is it what is explained after?
so
...
%LORENZO ". The correlated
%error model and an independent error model therefore describe the same total error
%for a single observation, and differ only in how that error is distributed between
%what the observations share and what is particular to each one."
% Totally not understandable What is the correlated error model, and the independent error model? Why do we need both=
...
%LORENZO: very obscure and technical. What is a triangular solve? Why do we need this covariance matrix? What does it mean to factorise it? Discuss and explain to me.

The first campaign carries the larger of the two campaign standard deviations, and that
is the only way in which it is treated differently from the other two. 
...
%same reason.
%LORENZO: no need to talk about the earlier version, but we need to explain why the first has the larger deviation. Those are just assumptions I think, but they are still reasonable and the rationale behind them should be properly discussed
...
with snooker update of \citet{terBraakVrugt2008}, 
%LORENZO: never heard "snooker update", might be fine but like this sounds jargon, maybe explain in half a line or one line max what it means in practice or why is it a good choice...
in the implementation provided by the
...
conventional calibration, and the reason it is not free is worth stating. 
%LORENZO: "Reaching the observed stocks is not the free parameter" does not make much sense, logically. REaching an observed stock is not a free parameter in any case, the phrase sounds very absurd.
...
would be arithmetic rather than a result. 
%LORENZO: not understanding this statement. We don't want to reach one stock, but we want our simulation to represent three stock measurements in time. What steady state has to share with this?

Neither the litter input nor the transit
...
measurements. 
%LORENZO: definitely I don't get the point of this. Also above we say that we don't need that freedom since transfer coefficients and rates are almost equivalent due to model equifinality. But most importantly I don't understand what all this discussion is meant to mean nor whit it is here. explain and discuss.

What the six models are asked to do is to reach three observed stocks,
...
%LORENZO: "That all six models do so is evidence that the frame in which they are compared is fair." why? I don't reallythink so, i's just evidence we are calibrating some models with enough freedom, nothing particularly big or surprising?
...
%LORENZO: ok, correct me if you need to but I don't think most of this paragraph is useful. Our calibration manages to reproduce the trends. This is not entirely surprising, but maybe it is a good thing and jus worth noting because the trends are not so simple to reproduce with the inputs we have, maybe?  So our transient initialization idea might have something to share with this achievemtn? And you seem developing this idea later.
...
for three reasons: the models were not selected for that property, 
%LORENZO: which property? Falling close to the observed rate? Maybe far fetched, would not say this one
the observed
...
%LORENZO: "the spread between models is a diagnostic rather than an estimate of uncertainty.": a bit self reliant as statement, hanging nowhwere. I mean, if we say so? But this is not how science should work. To me the measurement uncertainty is the only one that stands, and it is a good reason not to overinterprete that result. We are not certain of the "true" SOC trajectory to an extent that allows us to select the models with that precision.

The comparison with the observed rate is reported as a consistency check and not as
a validation, for two reasons. 
%LORENZO: this seems to introduce more or less a repetition of the above. If rates and stocks are carryuing different information (I doubt so if interprteting the statement literally, but to some extent this is still true), we should first introduce this difference and why we now talk about rates
The first reason is that the three campaigns enter the
calibration, so the observed rate is not independent of the fit. 
%LORENZO: It says that they match the rate because they were calibrated on measurements that match the rate. Uninformative. But true, indeed.
A review of some two
...
predict one. 
%LORENZO: ok, this makes sense. It basically says we might be overfitting, we are calibrating against these data so of course the model will tend to represent them, we are just consistent with them. Worth saying it but on a side, this is not very very informative.
The second reason is the observations themselves. The three campaigns were
...
%LORENZO: this mirrors the discussion above about the stocks, and of course it does, it is talking about the same thing. I would drop the distinction between rates and stocks, they are two sides of the same. Maybe just discuss these two issues (we are calibrating on these data so we expect the model to match them, to some extent; and second, we cannot claim the measurement are absolute truth either), as caution against overintrepretation. Good thing: our approach can represent a complex trend not totally justified by the inputs, so it works for what we designed it.
I would put toghether this paragraph and the one before, stripping it of all the fuzzy uninformative fillers. Please discuss this with me, the story and what this merged paragraph should say as information content boiled down.
...
then decline.
%LORENZO: and yes this is very interesting! The models manage more or less to represent the data, but when we extrapolate we might have surprises. This is leading to important thing for us.

The shape of the trajectory is the property of interest because it is what we would base our extrapolations upon, and the shape is precisely
what a starting state in balance discards. 
%LORENZO: it does not "discard" it, it will still have a shape also starting at steady state at time zero! It just does not have the same freedom to represent the observed trajectory, would probably struggle, and we should explain how and why it would struggle.

Reproducing an accumulation that slows down is therefore the first result
of this study, and reproducing it is only possible because the starting state was
estimated rather than assumed.
%LORENZO: no, totally not. It was possible because we have a transient initialization, and the model is not starting from steady state. It manages therefore to reproduce the initial increase and leveling off because it reproduces the disequilibrium we hypothesize it was starting from due to the historical reasons (and here we reconnect to that topic).

\subsection{Model structure is nearly invisible in the fit}

Skill does not increase as pools are added. 
%LORENZO: correct but a bit dry, I would start fromthe results, and then go towards this conclusion,. not frontloading this one.
...
%LORENZO: here connect with the above. Basically we cannot really distinguish the six models based on the data, for what stated above, so we cannot say that more complex models are any better in terms of pure fitting the data. THat's it, no more.
...
choose between these model structures. 
%LORENZO: not only because of that, but because of the problems stated above when talking about the fit to SOC stocks and rates. Connect there. here you are just repeating the same, just introducing the complexity of models into that. Maybe also this could be put together in the sameparagraph, after saying that we cannot really diustinguish the models based on fit with these data (also because of all the local variance, btw, add also this one! Our models represent the general trends but not the local variance, none of them, and this should be discussed in the beginning of that section where we say that fitness alone does not allow us to choose models).

Whatever separates the six structures, the
...
%LORENZO: yes, definitely a repetition
...
%LORENZO: is "nat" explained before as unit of measurement? If not it must be.
...
%LORENZO: bottom line is that we can represent the trends but parameters are still undetermined, right? This above could be a good conclusion of the paragraph collapsing everything until this point
...
%LORENZO: and ok, this is a new paragraph, definitely. From here on we start another one.
...
not have}
%LORENZO "the models describe a gradient the soil does not have" is pretty negative and just seems to say that models do not work
...
%LORENZO: I think this would be far fetched, as the title frame it. But what it MIGHT mean in general, for SOC modeling and their applications, would be a similar framing that is something I would consider reasonable
...
of all three campaigns, 
%LORENZO: again I do not think the stocks are the point here, since we have three points in time. "Reaches the stock" (particularly "reaches") means then I would ask "when? TO which of the campaings do you refer to?". But the model represent well all the observations, over time, all the campaigns.
the six models bracket the observed rate of change, and the
...
Reproducing the observed trajectory requires 
%LORENZO: does it? "Requires"? I think the ridge points in that direction, but that if we moved across it, towards values more similar to the original parameterization, we would have still a quite similar fit, not exactly but quite similar. This is not a strong evidence, it does not requires it. But our system with all its assumptions, hints, gently, in that direction.
THIS IS PRETTY IMPORTANT, STORY NEEDS TO BE REVISED TONING DOWN THAT CLAIM. Not much change needed, it is already good, just too strong. Implement all along.
more litter input, and faster turnover,
...
structures and not for others. 
%LORENZO: "reaches the projection for some model structures and not for others." not clear at all, do you mean it has implication for some of the extrapolations? 
A model that applies one climate modifier to every
...
%LRENZO: no, this seems to say that we can accumulate SOC. One of our conclusions is that soils are indeed saturating, and that is politically very loaded, we must not do mistakes or be ambigious on that. Here we are telling them that soils are indeed saturating, but maybe a bit faster than expected (Liski 2006), according to our results. Or same? Is it faster than expected? If we are instead confirmi Liski 2006, also that is a very relevant result.
...
conclusion about how far the soil is from balance. 
Realising that room, however,
...
management that such a change implies. 
%LORENZO: "Realising that room" is very bad way of saying it, but most importantly holding and increasing are completely different scenarios. Holding it means that we are approaching saturation sooner (and how soon is important to them, crucial), and we can expect to see less and less sequestration. Increasing it instead is what has a big political and economical cost, and they want to know how much sequestration it can buy us and for how long. Probably not too long, but we shouldprovide some estimate (e.g: what would happen if we increased inputs by 20%? This might require a bit more analyses).
Realising that room would also postpone
...
%LORENZO here we can say straight out that this is a baseline for further model development since it tells us what we might want to include and where our models might currently fall shoret to predict that variance
...
%LORENZO: maybe this is not in conclusions, but before, one of the last paragraphs before limitations.
...
%LORENZO also code will get a Zenodo link, so that it is static, but this will be the very last part
