...
same width of $0.25$; the reconstruction of \citet{Liski2006}, based on the same forest
record, implies a similar ratio, about $0.86$. Both parameters are estimated together
...
\gap{Discuss the starting state (option a, 2026-09-23): the posterior of
$\sigma_{\mathrm{init}}$ is $0.49$--$0.59$ in five models ($0.84$ in SP1), well below the
prior centre $0.90$ and the $0.86$ implied by \citet{Liski2006}. The calibration places
the 1917 input at about half the 1985 input. $\sigma_{\mathrm{init}}$ is the only
parameter that can lower the 1917 stock, so it also lowers the 1917 input; one
parameter cannot express a high input on a soil that was itself out of balance after
the exploitation described in the Introduction.}
...
\gap{The gridded soil carbon maps and their uncertainty. One short paragraph, framed
as a deliverable and not as a finding.}
...
pre-observation litter ramp, including, in one short sentence, that the tree-derived
terms of the reconstruction cover all forest land while the area used to divide them
covers upland soils only, so the rise of the pre-observation input is, if anything,
overstated; the single national initial state where the depletion
history was regional; the 1917 carbon stock implied by the models, which is reported
rather than constrained; and the fact that the main projection contains no climate
change. (2026-09-23: the passage about our own earlier level offset is DROPPED, since the
paper does not refer to its own development history.)}
...
project falls to about half within $60$ years, because most of the remaining distance
lies in the slowest pool and is realised only over centuries. A larger litter input
