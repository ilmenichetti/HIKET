...
The calibration set is $447$ permanent forest plots drawn from the Finnish
...
Each plot carries mineral-soil organic carbon stocks from up to three
...
F/H~layer, or with an implied mean residence time $>100$~yr are excluded. The
...
by Yasso: a non-woody component (foliage and fine root) plus fine- and
coarse-woody components. It is a \emph{tree}-litter product --- understorey and
ground-vegetation inputs are not represented, which becomes relevant to
$\sigma_{\mathrm{input}}$ below. The population mean rises from $\sim\!2.1$ to a mid-2000s peak of
...
All six models see exactly the same inputs: the same $447$ plots, the same
...
homogeneous in method and scale rather than magnitude; (ii)~the twelve
inter-pool transfer fractions share a common weakly-informative logit prior
(SD~$0.4$) with per-model published centres, wired by explicit per-fraction
listing; and (iii)~the auxiliary parameters $\sigma_{\mathrm{init}}$ and
...
% ROUND-1 REVISION: the simple-model rates are now EXTERNALLY ANCHORED (Ultuna
% bare-fallow ICBM), not left loosely calibrated. This closes the earlier rate
% asymmetry: every model now carries external kinetic constraint of comparable
% weight (litterbag for Yasso, ICBM for the simple ladder), so the ladder is on
% equal INFORMATION footing, not merely equal degrees of freedom. Numbers/tables
% land with the production recalibration; see REVISION_PLAN.md §C1.
The fair frame requires not just equal degrees of freedom but equal external
\emph{information}. The inter-pool transfer fractions are equal-DoF --- all free,
under the common logit prior --- in every multi-pool model, so no model splits a
pool's outflow more freely than another. The base decomposition rates are
\emph{anchored to external calibrations} in every model, of comparable
evidential weight: Yasso's rates come from its litterbag calibration (Tuomi et
al.\ 2009/2011), and the simple-model rates are anchored to the Ultuna
bare-fallow ICBM calibration (Andr\'en \& K\"atterer 1997; $k_1=0.8$,
$k_2=0.00605$~yr$^{-1}$, humification $h=0.13$). How completely a structure can
be anchored is set by whether its time-scale structure matches an existing
calibration: TP2 (two time-scales) maps onto ICBM one-to-one; SP1 (one
time-scale) is anchored to the ICBM bulk residence time; and TP3 (three
time-scales) anchors its fast pool and its slow-subsystem \emph{aggregate}
residence time to ICBM, leaving only the internal slow/humus split --- the one
genuinely non-identified degree of freedom --- free. The auxiliary parameters
($\sigma_{\mathrm{init}}$, $\sigma_{\mathrm{input}}$) and the climate response
remain calibrated in every model. Thus each model spends its freedom on the same
things (fractions, inputs, initial state, climate) and pins its rates to the
best external evidence available for its structure; identifiability then tracks
information, by design rather than by accident.
...
product is a tree-litter model that structurally omits understorey/ground-vegetation
and mycorrhizal inputs, the $\sigma_{\mathrm{input}}$ prior is centred on the
physically expected correction ($>1$) rather than on one, so its posterior is
read against expected total litter rather than against the tree-only baseline.
...
\begin{itemize}
\item The litter product used here is \emph{tree-litter only}; understorey/ground
  vegetation and mycorrhizal inputs are not represented. So a multiplier above one is
  a physical expectation, not manufactured carbon --- hence the prior is re-centred at
  $\sim$1.3 rather than 1.
