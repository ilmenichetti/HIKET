...
\usepackage{xcolor}
\newif\ifstrat
% DROPPED 2026-09-02 (Lorenzo): the stratified-calibration strand is not part of
% THIS publication. Possibly revisited later, so the material is kept behind the
% switch rather than deleted -- set \strattrue to bring it all back.
\stratfalse       % <-- \strattrue restores the entire stratified-calibration strand
...
% ⚠ TWO published-MRT bases exist (POINT vs the .dat posterior); both are given.
% ⚠ CORRECTED 2026-09-02: Yasso20's published figures were a CHIMERA -- assemble_params
% keeps a fixed-rate block that for Yasso20 holds YASSO15's rates and w parameters, so a
% published draw never overrode them. Point 19.03 -> 21.85, posterior 25.04 -> 22.13.
% The old text read "Yasso20 sits entirely above its published point", which is now
% trivial rather than a finding. See doublechecks/intrinsic_mrt.R.
...
($19.6$--$26.4$). The corresponding published values are $33.5$, $30.5$ and
$22.1$~yr. Two of the three are displaced and one is not: Yasso07 falls well
short of its published value and Yasso15 moderately so, while \textbf{Yasso20 is
within $2\%$ of its own} --- its published parameterisation already sits where
our calibration puts it. That split is not incidental, and Section~[Discussion]
returns to it: the models whose turnover moves least are also the ones whose
projections prove least sensitive to the input assumption.
...
calibration cannot exclude local kinetic variation, and the fair test of that ---
a spatially stratified calibration --- is beyond this paper. Second --- and this is the
