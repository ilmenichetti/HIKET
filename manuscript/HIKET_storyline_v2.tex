...
\begin{test}
\textbf{Design.} Two axes, one readout.
\smallskip

\begin{center}\small
\begin{tabular}{ll}
\hline
\textbf{Axis 1 --- parameters} & published (Yasso07/15/20 defaults, $\sinput = 1$) \\
...
\end{center}
\smallskip
\textbf{Uncertainty.} Bootstrap over MCMC draws for ours; over the FMI \texttt{.dat} draws
($n = 400$) for published Yasso15/20; Yasso07 has a published \emph{point} only, so it enters as a
line against our band. The machinery for building published parameter vectors already exists and is
proven in \texttt{doublechecks/intrinsic\_mrt.R}.
\end{test}

\paragraph{Read the \emph{response}, not the trajectory.} The two parameter sets start from
different stocks, so absolute trajectories differ for two reasons at once. The scenario-minus-control
contrast removes the initial state and isolates reactivity.

\subsubsection{The one design decision to settle first}
\paragraph{What initial state does the published arm get?} Three defensible answers, and they ask
different questions:
\paragraph{(a) Shared state.} Both start from the same stock. Isolates kinetics cleanly; artificial.
\paragraph{(b) Own transient init.} Each parameterisation initialises itself the way we do. Fair
comparison of parameters, holding method constant.
\paragraph{(c) Own \emph{steady-state} init.} The published arm gets the convention it was actually
used with. This is the honest ``what would the conventional setup have projected'' --- but it moves
two things at once, so it answers the paper's \emph{other} question, not this one.
\begin{note}
\textbf{Suggestion:} (b) as primary --- it isolates the parameter displacement, which is what this
test is for --- with (c) reported alongside, because the difference between (b) and (c) is precisely
the initialisation effect the first half of the paper is about. (a) as a diagnostic only.
\end{note}
