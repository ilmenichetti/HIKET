...
\hline
& \multicolumn{2}{c}{published arm} & \multicolumn{2}{c}{our arm} & \multicolumn{2}{c}{misfit (rms log)} \\
& MRT & $\sinput$ & MRT & $\sinput$ & published & ours \\
\hline
Yasso07 & 33.47 & 1.249 & 24.57 & 1.585 & \textbf{0.020} & 0.029 \\
Yasso15 & 30.44 & 1.324 & 27.11 & 1.538 & \textbf{0.024} & 0.034 \\
Yasso20 & 22.13 & \multicolumn{5}{l}{\emph{blocked by our integrator --- see below}} \\
\hline
\end{tabular}
\end{center}
...
not level mismatch --- which is exactly the condition the experiment required.

\paragraph{Both arms are on the same ridge.} The product $\mathrm{MRT} \times \sinput$ is conserved
to within $7\%$ (Yasso07 $41.8$ vs $38.9$; Yasso15 $40.3$ vs $41.7$). We are moving \emph{along} the
ridge, not off it --- which is what makes the contrast interpretable.

\paragraph{\textbf{Yasso20 is blocked by our integrator, not by its structure.}} Published
Yasso20 routes $\sim\!99.6\%$ of pool A and $\sim\!98\%$ of E into W. Our Yasso wrapper's
diagonal-dominance guard falls back to explicit Euler near that boundary, and with
$\alpha_W \approx 4.4$ and $\xi$ up to $3.4$ the Euler factor is about $-14$, so the solution
oscillates and diverges. It is a \emph{threshold}: $p_{AW} = 0.9926$ (ours) integrates fine,
$0.9957$ (published) diverges --- a change of $0.003$, with outgoing sums $\le 0.996$ in both, so
mass conservation is intact throughout. \textbf{FMI's own Yasso20 does not do this}; the fix is to
make our step exact there, not to drop the model. Until then the experiment runs on Yasso07 and
Yasso15.

\begin{warn}
\textbf{F12 overstates published Yasso20's turnover by $11.6\%$, and this must be fixed before the
figure is used again.} \texttt{assemble\_model\_params} builds
\texttt{c(fixed\_rates, p\_free[MODEL\_FREE\_NAMES])}, and for Yasso20 the \emph{rates are fixed at
Yasso15's values} --- they are not free names, so a published draw never overrides them. F12's
``published Yasso20'' therefore pairs Yasso20 fractions and climate with Yasso15's $\alpha_E$
($0.242$ against the published $0.117$) and Yasso15's non-zero $w_1$--$w_5$ (published Yasso20 sets
them to zero). Corrected: $25.04 \to \mathbf{22.13}$~yr. Yasso15 is unaffected ($30.44 \to 30.54$,
$+0.3\%$) because its fixed rates \emph{are} its own published ones.
\smallskip

\textbf{Consequence for the story:} corrected published Yasso20 ($22.13$) is within $2\%$ of ours
($22.56$). \textbf{Yasso20's turnover is not displaced from its published value at all}, and any
claim that it is --- including ``Yasso20 now exceeds its published point'' --- must be restated.
\end{warn}

\begin{note}
\textbf{Retracted 2026-09-02:} an earlier version of this section claimed Yasso20 ran ``backwards''
--- shorter published turnover and higher published input than ours --- and offered it as a built-in
sign control. That came from using \texttt{to\_original(best\_x)} as ``the published
parameterisation''. For Yasso20 that vector is a \emph{hybrid}: Yasso20 climate and size parameters
with \textbf{Yasso15's transfer fractions}, because that is where its prior centres come from
(\texttt{Prior\_specs/Yasso20\_priors.R}, line 3). Its MRT is $19.03$ against the genuine published
$25.04$. On the correct basis the family is ordered consistently --- published slower than ours in
all three --- which is what \texttt{F12\_mrt\_yasso.png} showed all along.
\end{note}

\begin{note}
\textbf{What this already tells us, before any forward run.} On the aggregate trajectory,
slow-and-lean and fast-and-rich are \emph{indistinguishable}. That is the ridge demonstrated
directly rather than inferred from a posterior correlation --- and it \textbf{bounds what the input
claim of Act VI may rest on}. The national trajectory does not support it; the plot-level
likelihood does. The manuscript must say which.
\end{note}
...
\NEW{F15 --- climate response (warming $-$ control) at 2084, published vs ours, three Yassos,
posterior bands}
...
\paragraph{To build.} F3b (shape groups), F15 (climate reactivity, arm A vs arm B), the variance
partition schematic (optional).
