...
Yasso20 & 25.04 & \multicolumn{5}{l}{\emph{not constructible --- see below}} \\
\hline
\end{tabular}
\end{center}
...
\paragraph{\textbf{Yasso20 cannot enter this experiment as published.}} Its published
parameterisation (Viskari 2022) fixes \textbf{six transfer fractions at structural zero} and derives
\textbf{three more algebraically}; ours frees all twelve. The published vector therefore has no image
in our parameter space --- assigning the published columns leaves the three derived fractions
uncomputed, and every one of the $10{,}000$ published draws diverges in forward integration. The
experiment runs on \textbf{Yasso07 and Yasso15}; Yasso20 is reported as a structural limitation.

\begin{warn}
\textbf{This also puts a question mark over F12's published Yasso20 band.} That band
($25.04$~yr) is computed from the same raw draws through the steady-state route, which returns
finite values where the forward run does not. Whether a steady state built from an inconsistent
fraction matrix is meaningful must be checked before F12 is used again.
\end{warn}
