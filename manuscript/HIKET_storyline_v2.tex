...
\begin{note}
\textbf{This is the beat the old note did not have.} Previously ``inputs are the binding
uncertainty''. Now something sharper and more falsifiable: the published parameterisations appear to
\emph{underestimate both} the litter reaching the soil and the rate at which it decomposes --- and
that is a statement about \emph{where on the ridge} the truth sits, which has consequences.
\end{note}
...
position on the ridge an empirically decidable question rather than a prior-driven one.
\end{note}
...
\subsection{The test can be run now}
\begin{test}
\textbf{Arm A vs arm B is the two-calibration experiment.} The $\sinput$ anchor sensitivity produced
exactly the required design --- the same six models calibrated under two input assumptions, both on
disk with chains:
\smallskip
...
\begin{tabular}{lccc}
\hline
 & arm A (Liski, centre 1.27) & arm B (L\&H, centre 1.08) & shift \\
\hline
$\sinput$ (Yasso15) & 1.619 & 1.538 & $-5\%$ \\
MRT Yasso07 & 22.13 & 24.57 & $+11\%$ \\
MRT Yasso15 & 25.60 & 27.11 & $+6\%$ \\
MRT Yasso20 & 20.82 & 22.56 & $+8\%$ \\
effective flux & 3.39--4.73 & 3.19--4.34 & lower \\
fit & --- & \emph{marginally better} & --- \\
\hline
\end{tabular}
\end{center}
\smallskip
Two positions on the ridge, comparable fit, no new runs required. Apply the climate-sensitivity
machinery to both and read off the difference.
\end{test}

\paragraph{Pre-registration, before looking.} Structure predicts \emph{where} the effect should
appear: Yasso07 large (single $\xi$ ties MRT and climate sensitivity together; corr $-0.58$),
Yasso15/20 small (three pool-specific $\xi$ decouple them; corr $-0.10$/$-0.16$).

\paragraph{What would falsify the bet.} A uniform, negligible difference across all three Yassos.
That would mean the ridge is degenerate in climate response too, and the position on it is
\emph{not} empirically decidable from projection --- a real result, and one worth reporting.

\NEW{F15 --- climate reactivity, arm A vs arm B, per model, posterior bands}

\subsection{Stronger versions, if the cheap test pays off}
\paragraph{Forward runs rather than a step response.} Replace the single-exponential demonstration
with actual forward integration under a warming scenario.

\paragraph{Deliberate ridge-walking.} Calibrate under a restrictive and a permissive $\sinput$
prior --- wider separation than the anchor arms give.
