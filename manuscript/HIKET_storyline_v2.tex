...
% --- figure embedding (2026-09-03) ---------------------------------------
% \FIG{basename} keeps the inline blue marker AND places the actual figure, so the
% storyline can be read against its evidence. Resolution order:
%   figures/<basename>.png  -> image float
%   figures/<basename>.tex  -> \input as a table float
%   neither                 -> amber "missing" marker, nothing placed
% The catcode dance lets call sites write raw underscores (F14_mrt_ridge), which is
% what the filesystem needs; \FIGlabel prints the name safely either way.
\usepackage{placeins}
...
\newcommand{\FIGaux}[1]{%
  ...
  \IfFileExists{figures/#1.png}{%
    \begin{figure}[htbp]\centering
      \includegraphics[width=0.92\linewidth,height=0.42\textheight,keepaspectratio]{figures/#1.png}
  ...
  \IfFileExists{../Reporting/NextgenC_report/SOC_maps/thumbnails/#1.png}{%
    \begin{figure}[htbp]\centering
      \includegraphics[width=0.62\linewidth,height=0.44\textheight,keepaspectratio]{#1.png}
      \caption{\textsf{#1} (NextGenC deliverable)}\end{figure}}{%
  \textcolor{warnrule}{\small[\textsf{#1} --- FILE NOT FOUND]}}}}}
% A figure reference that must NOT place anything (illustrative, or a script).
...
the input/turnover ridge. Those are the four \texttt{\textbackslash part} divisions.
\textbf{Inside the Resolution the order is deliberate}: fit cannot arbitrate, and the data pin only a
level --- so by the time the ridge arrives, the displacement is unavoidable rather than one finding
among three. The ridge then does double duty: it \emph{closes} the Resolution and \emph{opens} a side
arc (its own \texttt{\textbackslash part}), which asks whether sitting elsewhere on it has
consequences. Landing, Coda and Appendix follow.
\end{note}

\begin{note}
\textbf{How to read this.} Every block is a claim we intend to make, its evidence, and nothing
else. \FIGX{name} marks a figure that exists; \NEW{name} marks one that does not yet.
...
one: exploitation, then a century of managed regrowth. \FIG{F10b_forest_history}
...
``$26$~tC\,ha$^{-1}$ too high at 1985'' caveat is gone. \FIG{F3_mean_soc}
...
\section{Fit cannot arbitrate between the structures}
\begin{note}
...
\FIG{T1_model_summary} \FIG{F6b_skill_vs_complexity}
...
\paragraph{The forecast diverges where the calibration agreed.} \FIG{F4_initialization} (panel b)
\begin{warn}
\textbf{Both drivers are held stationary in this projection, and the paper must say so.} Climate is
the last twenty observed years recycled cyclically over sixty; litter input is frozen at its 2024
value. So the projected sink is the models finishing the accumulation they were initialised into ---
relaxation toward equilibrium, not a response to anything --- and its magnitude inherits the
historical input reconstruction (it correlates $r=+0.87$ with each model's inferred historical litter
rise, which independent evidence says is $4$--$6\times$ too steep).
\textbf{The claim made here survives that}: with no external forcing the spread between models is
\emph{purely structural}, which is cleaner evidence for this paragraph than a scenario run would be.
What must not be read off panel~b is the absolute sink. Climate change is out of scope for the main
forecast by choice; the side arc explores it as a consequence of the ridge position, and doing it
properly needs far more than bolting scenarios on --- at minimum letting productivity respond to
warming, which currently biases every arm toward loss.
\end{warn}

\begin{note}
\textbf{The $+2$\,\textdegree C equilibrium sweep was CUT here on 2026-09-03.} It reported
``climate sensitivity differs $\sim$2$\times$ across the family'' from a step change and a
single-exponential on bulk MRT, measured on a superseded run. Two reasons it goes: the side arc
measures the same quantity properly (transient, current run, real scenarios), and the side arc's own
collapse test \emph{refutes} the sweep's central assumption --- bulk MRT does not determine the
warming response ($R^2=0.068$ within Yasso20). Keeping both would put a superseded method in the
main text and its refutation in the appendix.
\end{note}
...
\section{The data pin a level, and little else}
...
\section{So the displacement is real, and it is joint --- the ridge}
\begin{note}
...
total including understorey ($\sim$2.7). \FIG{SX_input_vs_literature}

\paragraph{Independent corroboration exists.} Liski 2006 --- same country, period, model family and
inventory --- implies a comparable correction. \FIG{F1_inputs}
...
\FIG{F14_mrt_ridge} \FIG{S13_mrt_ridge_benchmark}
...
\begin{note}
\textbf{Why this belongs with the ridge rather than beside it.} The evidence does not ride the
...
% ---------------------------------------------------------------------
% THE HINGE. Load-bearing: the same beat must CLOSE the Resolution and OPEN the
% side arc. If it reads as a section break, the reader reads two papers.
% ---------------------------------------------------------------------
\begin{note}
\textbf{The hinge --- draft it early, it carries the whole join.}\\[3pt]
\emph{Resolution:} transient initialisation works --- the models reach the observed levels and two
reproduce the rate --- \textbf{and} it puts the ensemble at a point on the input/turnover ridge
where no published parameterisation sits.\\[3pt]
\emph{Opening:} the data cannot choose between that point and the published one, because the ridge
is flat in fit. So the question the result raises is not \emph{which point is right} --- nothing here
can answer that --- but \textbf{whether the choice matters}. That is answerable, and it is what the
next part does.
\end{note}

% =====================================================================
% SIDE ARC. Opened by the ridge, closed by the forward experiment. Deliberately a
% separate \part: it is a consequence of the main result, not part of the main
% forecast. Climate change is OUT OF SCOPE for the main line (see the F4 caveat);
% here it is the instrument for asking whether ridge position has consequences.
% =====================================================================
\part{Side arc \textnormal{---} does the ridge position matter?}
...
\FIGX{doublechecks/published\_arm\_dat.R} \FIG{T_A1_published_arm}
...
\FIG{A1_forward_appendix}
\FIG{T_A1_published_arm}
...
% =====================================================================
\part{Landing \textnormal{---} the policy deliverable}

\section{What we hand over}
...
\paragraph{Maps as the landing figure.} \FIG{ENSEMBLE_SOC_mean}
