# =============================================================================
# run_SP1_predictive.R
#
# Posterior predictive stage for SP1 -- HIKET intercomparison.
# Mirrors run_Yasso07_predictive.R exactly except:
#   - sources sp1_wrapper.R (no dyn.load; pure R model)
#   - assemble_model_params is a pass-through (all params free)
#   - engine wrappers call sp1_* functions
#   - posterior_predictions stores pool column C (not A/W/E/N/H)
#   - no respiration column (sp1_run does not compute it)
#
# USAGE:
#   Rscript run_SP1_predictive.R <RUN_ID>
# =============================================================================
...
if (length(args) >= 1) {
  ...
  if (is.null(RUN_ID)) {
    ...
    if (length(rns) == 0)
      stop("No SP1 calibration posterior found in ./Calibration_real_data/runs/")
...
# =============================================================================
# 2.  Parameter assembly (pass-through: all SP1 params are free)
# =============================================================================
...
t_pp <- system.time({
  posterior_predictions <- do.call(rbind, lapply(seq_len(N_PP_DRAWS), function(d) {
    ...
    do.call(rbind, mclapply(plots_real, function(pid) {
      ...
      xi_for_ss <- tryCatch(
        compute_xi_mean_sp1_engine(clim[seq_len(n_ss), , drop = FALSE], model_params),
        error = function(e) NULL)
      ...
      # SP1 output: year, C, total_soc  (single pool; no respiration)
      data.frame(
        plot_id   = pid,
        year      = run_out$year,
        draw      = d,
...
plot(NA, xlim = range(traj_summary$year), ylim = ylim_r,
     ...
     main = sprintf("Mean ΔSOC across plots  |  %s  |  %s", MODEL_NAME, RUN_ID))
...
legend("topleft",
       legend = c("Posterior median","50% CI","95% CI",
                  sprintf("Observed mean ΔSOC ± 95%% CI  (n = %d)",
