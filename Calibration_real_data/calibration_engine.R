...
# =============================================================================
# 3.9  Xi validity helper
#
#      PURPOSE:
#        The xi_for_ss object returned by compute_xi_mean differs in structure
#        across models: a scalar for Yasso07, a named list (xi_awe, xi_n, xi_h)
#        for Yasso15 and Yasso20. This helper validates both forms so the
#        likelihood's guard clause stays model-agnostic.
#
#        A scalar xi is valid if it is finite and strictly positive.
#        A list xi is valid if every element is a length-1 numeric that is
#        finite and strictly positive. Length-1 is enforced because
#        compute_xi_mean is always called on the steady-state window reduced
#        to its mean climate -- it must produce a single scalar per pool group.
# =============================================================================

is_valid_xi <- function(xi) {
  if (is.numeric(xi)) {
    length(xi) == 1L && is.finite(xi) && xi > 0
  } else if (is.list(xi)) {
    all(vapply(xi, function(v)
  ...
  cmpfun(function(x) {
    ...
    log_liks <- parallel::mclapply(plots, function(pid) {
      ...
      if (is.null(xi_for_ss) ||
          !is_valid_xi(xi_for_ss))              return(-Inf)  # degenerate xi
...
run_diagnostics <- function(chain_results, to_original, param_names,
  ...
  # ==========================================================================
  # --- Visual diagnostics: traces, R-hat barplot, marginals, KL barplot ---
  # ==========================================================================

  PX_PER_IN   <- 150L
  chain_cols  <- c("steelblue","firebrick","darkgreen","darkorange")[seq_len(N_CHAINS)]

  # Parameters shown in trace plots: highlight set or all if not specified.
  plot_params  <- if (is.null(highlight_params)) param_names else highlight_params
  n_pp         <- length(plot_params)
  n_cols_tr    <- 2L
  n_rows_tr    <- ceiling(n_pp / n_cols_tr)

  # ------------------------------------------------------------------
  # Prior samples in physical space.
  # Generated COLUMN-BY-COLUMN to avoid the rnorm() recycling bug:
  # vectorised rnorm(n*N_FREE, mean=best_x, sd=sigma_ppm) recycles
  # element-wise through the flat output, scrambling which (mean, sd)
  # pair each sample actually uses. Looping over j fixes this.
  # The same prior_phys matrix is reused for marginals AND the KL plot,
  # so it is computed once here before any PNG is opened.
  # ------------------------------------------------------------------
  set.seed(99)
  n_prior   <- 3000L
  ...
  # ------------------------------------------------------------------
  # Trace plots (highlight parameters only)
  # ------------------------------------------------------------------
  trace_file <- file.path(DIR_DIAG,
                          sprintf("%s_traces_%s.png", MODEL_NAME, RUN_ID))
  png(trace_file,
      width  = 10L * PX_PER_IN,
      height = max(4L, n_rows_tr * 2L) * PX_PER_IN,
      res    = PX_PER_IN)
  par(mfrow = c(n_rows_tr, n_cols_tr), mar = c(4, 4, 3, 1))
  for (nm in plot_params) {
    ...
    plot(traces[[1]], type = "l", col = chain_cols[1], ylim = ylim_r,
         xlab = "Post-burnin iteration", ylab = nm,
         main = sprintf("%s | %s", MODEL_NAME, nm))
    for (ci in seq_len(N_CHAINS)[-1]) lines(traces[[ci]], col = chain_cols[ci])
    legend("topright", legend = sprintf("Chain %d", seq_len(N_CHAINS)),
           col = chain_cols, lwd = 1, bty = "n", cex = 0.7)
  }
  dev.off()
  message(sprintf("Traces:    [%s]", trace_file))

  # ------------------------------------------------------------------
  # R-hat barplot: one bar per free parameter, coloured by convergence zone.
  #   Green  < 1.05  -- good
  #   Yellow 1.05-1.10 -- acceptable, watch
  #   Red    > 1.10  -- not converged, extend chain
  # Width scales with the number of parameters so labels don't crowd.
  # ------------------------------------------------------------------
  if (!is.null(gr)) {
    rv_all  <- gr$psrf[, "Point est."]   # named vector, all free params
    bar_col <- ifelse(rv_all < 1.05, "#4dac26",
               ifelse(rv_all < 1.10, "#f1b300", "#d01c8b"))
    bar_col[is.na(bar_col)] <- "grey60"

    rhat_png <- file.path(DIR_DIAG,
                          sprintf("%s_rhat_barplot_%s.png", MODEL_NAME, RUN_ID))
    png_w    <- max(8L, ceiling(length(rv_all) * 0.38)) * PX_PER_IN
    png(rhat_png, width = png_w, height = 6L * PX_PER_IN, res = PX_PER_IN)
    ...
    barplot(rv_all,
            ...
            las       = 2,
            cex.names = 0.78,
            ylim      = c(1, max(rv_all, 1.12, na.rm = TRUE) * 1.03),
            ylab      = "R-hat (point estimate)",
            main      = sprintf("%s | Gelman-Rubin R-hat  (%s)", MODEL_NAME, RUN_ID),
            xpd       = FALSE)
    abline(h = 1.10, lty = 2, col = "#d01c8b", lwd = 1.5)
    abline(h = 1.05, lty = 3, col = "#f1b300", lwd = 1.2)
    legend("topright",
           legend = c("Good (<1.05)", "OK (1.05\u20131.10)", "Warn (>1.10)"),
           fill   = c("#4dac26", "#f1b300", "#d01c8b"),
           border = NA, bty = "n", cex = 0.82)
    dev.off()
    message(sprintf("R-hat PNG: [%s]", rhat_png))
  }

  # ------------------------------------------------------------------
  # Marginal plots: prior vs posterior, split into two PNGs if
  # group1_params / group2_params are supplied by the calling script.
  #
  # group1_params: typically the compositional flow-fraction parameters
  #   (12 stick-break params for Yasso models). Plotted in a wider grid
  #   (up to 4 columns) so all 12 fit on one page.
  # group2_params: typically climate response + nuisance parameters
  #   (8 params). Plotted in a 2-column grid for larger panels.
  #
  # If neither group argument is provided, falls back to the original
  # single-PNG behaviour using highlight_params — preserving compatibility
  # with RothC and any other model added later.
  #
  # The x-axis for each panel is clipped to the posterior's 0.1%--99.9%
  # range unioned with the prior's 0.5%--99.5% range, so that heavy prior
  # tails after exp() or stick-break transforms do not compress the
  # posterior to an unreadable spike.
  # ------------------------------------------------------------------
  plot_marginal_group <- function(params, suffix, n_cols) {
    params <- intersect(params, param_names)   # guard against typos
    n_p    <- length(params)
    if (n_p == 0L) return(invisible(NULL))
    n_rows <- ceiling(n_p / n_cols)
    fpath  <- file.path(DIR_DIAG,
                        sprintf("%s_marginals_%s_%s.png",
                                MODEL_NAME, suffix, RUN_ID))
    png(fpath,
        width  = max(6L, n_cols * 3L) * PX_PER_IN,
    ...
    dev.off()
    message(sprintf("Marginals: [%s]", fpath))
    invisible(fpath)
  }

  if (!is.null(group1_params) && !is.null(group2_params)) {
    plot_marginal_group(group1_params, "fractions",
                        n_cols = min(4L, length(group1_params)))
    plot_marginal_group(group2_params, "climate_nuisance",
                        n_cols = min(4L, length(group2_params)))
  } else {
    plot_marginal_group(plot_params, "highlight", n_cols = 2L)
  }

  # ------------------------------------------------------------------
  # KL divergence barplot: KL(posterior || prior) per free parameter.
  #
  # Estimated by numerical integration of KDE curves on a shared
  # x-grid — the same grid used by plot_one_marginal_honest, so the
  # KL values are visually consistent with the marginal panels.
  #
  # Interpretation:
  #   ~0 nats  -- posterior stayed near the prior; parameter not
  #               identified by the data (data are uninformative here)
  #   ~1 nat   -- moderate information gain; posterior meaningfully
  #               tighter or shifted relative to prior
  #   >> 1 nat -- strong identification; data moved this parameter
  #               substantially away from the prior
  #
  # NOTE: KL is estimated on physical-space samples after the
  # reparameterisation. For stick-break parameters the physical values
  # live on a bounded simplex, so the KDE tails are clipped by the
  # density() bandwidth at the boundary — treat boundary-constrained
  # parameters' KL values as lower bounds.
  # ------------------------------------------------------------------
  kl_from_kde <- function(post_v, prior_v, n_grid = 512L) {
    ...
    xlim_prior <- quantile(prior_v, c(0.005, 0.995))
    xlim_post  <- range(post_v)
    ...
    # Clamp both densities away from zero before taking the log ratio.
    # The eps floor is much smaller than any meaningful density value so
    # it only affects the extreme tails where KDE mass is negligible.
    p  <- pmax(d_post$y,  1e-10)
    q  <- pmax(d_prior$y, 1e-10)
    dx <- diff(d_post$x[1:2])
    sum(p * log(p / q)) * dx   # KL in nats (trapezoidal integration)
  }
  ...
  barplot(kl_vals,
          ...
          las       = 2,
          cex.names = 0.78,
          ylab      = "KL divergence (nats)   KL(posterior \u2225 prior)",
  ...
  dev.off()
  message(sprintf("KL PNG:    [%s]", kl_png))
