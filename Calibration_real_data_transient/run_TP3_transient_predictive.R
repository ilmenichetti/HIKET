...
source("./Prior_specs/TP3_priors.R")   # TP3_ALPHA_A_FIXED (see Section 2)
...
# re-injected here to match run_TP3_transient_calibration.R -- without it the
# wrapper reads model_params["alpha_A"] as NA and tp3_transient_init fails for
# every plot. Constant = TP3_ALPHA_A_FIXED (Prior_specs/TP3_priors.R).
# alpha_S/alpha_H stay free (very-informative priors) and come from the posterior.
...
t_proj <- system.time({
  projection_predictions <- do.call(rbind, lapply(seq_len(N_PP_DRAWS), function(d) {
    ...
    do.call(rbind, mclapply(plots_real, function(pid) {
      ...
      run_up    <- tryCatch(tp3_run_engine(inputs_up, model_params, C_proj_init, xi_proj),
                            error = function(e) NULL)
      mp_eq     <- model_params; mp_eq["sigma_init"] <- 1
      xi_eq     <- tryCatch(compute_xi_mean_tp3_engine(clim_recycle, model_params),
                            error = function(e) NULL)
      C_eq      <- if (is.null(xi_eq)) NA_real_ else
        tryCatch(sum(steady_state_tp3_engine(mp_eq, equilibrium_lm(lm, inputs, RECYCLE_YEARS), xi_eq)),
