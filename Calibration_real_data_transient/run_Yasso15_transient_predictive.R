...
#
# Posterior predictive stage for Yasso15 transient calibration.
# Differs from run_Yasso15_predictive.R in:
#   - Sources yasso15_wrapper_transient.R (standalone, no external sources)
#   - DIR_RUNS / DIR_DIAG point to Calibration_real_data_transient/
#   - steady_state engine wrapper calls yasso15_transient_init
#   - assemble_model_params includes sigma_init (needed by transient_init)
#   - compute_xi engines use params vector (not individual beta/gamma args)
#   - precip merged into inputs_by_plot (required by yasso15_run)
#   - yasso15_run_engine passes xi_arrays (list) and precip correctly
#   - LITTER_COLS defined before yasso15_run_engine
# =============================================================================
...
if (length(args) >= 1) {
  ...
  if (length(rns) == 0)
    stop("No Yasso15 transient posterior found.")
...
POOL_COLS     <- c("A","W","E","N","H")     # Yasso15 five-pool state vector

# Litter columns scaled by sigma_input inside yasso15_run_engine
...
# =============================================================================
# sigma_init included so yasso15_transient_init can read it from model_params.

FIXED_RATE_NAMES <- setdiff(YASSO15_PARAM_NAMES,
...
assemble_model_params <- function(p_free) {
  ...
  c(yasso_params,
    ...
    sigma_init  = unname(p_free["sigma_init"]))   # needed by transient_init
}
...
# =============================================================================
# Yasso15 compute_xi takes a full params vector (not individual beta1/gamma).
# yasso15_run requires xi_arrays as a list (xi_awe/xi_n/xi_h) and precip.
...
# TRANSIENT: yasso15_transient_init replaces yasso15_steady_state
steady_state_yasso15_engine <- function(model_params, lm, xi_ss) {
  yasso15_transient_init(model_params, lm, xi_ss)
}

yasso15_run_engine <- function(inputs, model_params, C_init, xi_arrays, precip) {
...
t_pp <- system.time({
  posterior_predictions <- do.call(rbind, lapply(seq_len(N_PP_DRAWS), function(d) {
    ...
    do.call(rbind, mclapply(plots_real, function(pid) {
      ...
      # xi_arrays: list (xi_awe, xi_n, xi_h) for Yasso15
      xi_arrays <- tryCatch(compute_xi_yasso15_engine(clim, model_params),
      ...
      # Validate xi_for_ss list (xi_awe, xi_n, xi_h all positive and finite)
      ...
      run_out <- tryCatch(
        yasso15_run_engine(inputs, model_params, C_init, xi_arrays, clim$precip),
        error = function(e) NULL)
...
#   clim_proj$precip is passed directly to yasso15_run_engine (annual precip
#   is a column in the annual climate frame for Yasso15).
...
t_proj <- system.time({
  projection_predictions <- do.call(rbind, lapply(seq_len(N_PP_DRAWS), function(d) {
    ...
    do.call(rbind, mclapply(plots_real, function(pid) {
      ...
      run_proj <- tryCatch(
        yasso15_run_engine(inputs_proj, model_params, C_proj_init,
                           xi_proj, clim_proj$precip),
        error = function(e) NULL)
      ...
      run_up    <- tryCatch(yasso15_run_engine(inputs_up, model_params, C_proj_init, xi_proj, clim_proj$precip),
                            error = function(e) NULL)
      mp_eq     <- model_params; mp_eq["sigma_init"] <- 1
      xi_eq     <- tryCatch(compute_xi_mean_yasso15_engine(clim_recycle, model_params),
                            error = function(e) NULL)
      C_eq      <- if (is.null(xi_eq)) NA_real_ else
        tryCatch(sum(steady_state_yasso15_engine(mp_eq, equilibrium_lm(lm, inputs, RECYCLE_YEARS), xi_eq)),
...
message(sprintf("  R²: %.3f  |  RMSE: %.2f  |  Bias: %+.2f  |  ParamCov: %.3f  |  ppCov: %.3f",
                R2, RMSE_med, bias_med, coverage_95, pp_coverage_95))
# ParamCov is the spread of the MEAN across draws and omits observation error --
# expected to be small. ppCov is the TRUE posterior-predictive coverage and
# should sit near 0.95 if the error model is adequate.
