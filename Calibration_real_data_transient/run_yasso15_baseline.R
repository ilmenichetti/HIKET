# =============================================================================
# run_yasso15_baseline.R
#
# Deterministic forward run of Yasso15 at published default parameters.
# Reference baseline for comparison with calibrated posterior predictive outputs.
#
# Key differences from run_yasso07_baseline.R:
#   - Three pool-group climate responses (AWE, N, H); xi is a named list.
#   - yasso15_steady_state() requires xi_ss list and precip_mean.
#   - Annual precip merged into inputs_by_plot (leaching term, leac=0).
#   - 35-parameter vector; 11 fixed (alpha_*, p_H, alpha_H, w1-w5).
#
# Initialization: steady-state (sigma_init = 1.0 has no effect).
#
# OUTPUTS (diagnostics/Yasso15_baseline/):
#   Yasso15_baseline_predictions.rds
#   Yasso15_baseline_residuals.csv
#   Yasso15_baseline_plot_ranking.csv
#   Yasso15_baseline_mean_soc_trajectory.png
#   Yasso15_baseline_delta_soc_trajectory.png
#   Yasso15_baseline_obs_vs_pred.png
#   Yasso15_baseline_residuals_diagnostic.png
#   Yasso15_baseline_rf_importance.png
#   Yasso15_baseline_sampled_plots.png
#   Yasso15_baseline_worst25_timeseries.png
#   Yasso15_baseline_residuals_histogram.png
#   Yasso15_baseline_report.txt
# =============================================================================
...
MODEL_NAME <- "Yasso15"
DIR_OUT    <- "./Calibration_real_data_transient/diagnostics/Yasso15_baseline"
...
fixed_rates      <- YASSO15_DEFAULT_PARAMS[FIXED_RATE_NAMES]
MODEL_FREE_NAMES <- setdiff(names(YASSO15_FREE_DEFAULTS), c("sigma_init","sigma_input"))
free_defaults    <- YASSO15_FREE_DEFAULTS   # sigma_init=1, sigma_input=1 by construction
...
# Merge annual precip into inputs so yasso15_run receives it for the leaching term
# (leac = 0.0 makes it numerically inactive, but yasso15_run requires the vector)
...
# precip_mean in litter_means: mean annual precip over steady-state window,
# passed to yasso15_steady_state() for the Fortran leaching term
litter_means <- lapply(plots_real, function(pid) {
  ...
  list(
    ...
    precip_mean = mean(clim_ss$precip)
  )
})
...
predictions <- do.call(rbind, mclapply(plots_real, function(pid) {
  ...
  xi_arrays <- tryCatch(
    compute_xi_yasso15(clim$temp_mean, clim$temp_amplitude, clim$precip,
                       model_params[YASSO15_PARAM_NAMES]),
    error = function(e) NULL)
  if (is.null(xi_arrays)) return(NULL)

  n_ss      <- min(STEADY_STATE_YEARS, nrow(clim))
  xi_for_ss <- tryCatch(
    compute_xi_mean_yasso15(clim[seq_len(n_ss), , drop = FALSE],
                            model_params[YASSO15_PARAM_NAMES]),
    error = function(e) NULL)
  ...
  run_out <- tryCatch(
    yasso15_run(inputs_scaled, model_params[YASSO15_PARAM_NAMES],
                C_init, xi_arrays, precip = inputs$precip),
    error = function(e) NULL)
...
saveRDS(predictions, file.path(DIR_OUT, "Yasso15_baseline_predictions.rds"))
...
write.csv(residuals_df, file.path(DIR_OUT, "Yasso15_baseline_residuals.csv"),
          row.names = FALSE)
...
png(file.path(DIR_OUT, "Yasso15_baseline_mean_soc_trajectory.png"),
...
png(file.path(DIR_OUT, "Yasso15_baseline_delta_soc_trajectory.png"),
    width = 12L * PX_PER_IN, height = 6L * PX_PER_IN, res = PX_PER_IN)
par(mar = c(4, 4, 3, 1))
...
png(file.path(DIR_OUT, "Yasso15_baseline_obs_vs_pred.png"),
...
png(file.path(DIR_OUT, "Yasso15_baseline_residuals_diagnostic.png"),
...
png(file.path(DIR_OUT, "Yasso15_baseline_rf_importance.png"),
...
png(file.path(DIR_OUT, "Yasso15_baseline_sampled_plots.png"),
...
write.csv(plot_resid_summary,
          file.path(DIR_OUT, "Yasso15_baseline_plot_ranking.csv"), row.names = FALSE)
...
png(file.path(DIR_OUT, "Yasso15_baseline_worst25_timeseries.png"),
...
png(file.path(DIR_OUT, "Yasso15_baseline_residuals_histogram.png"),
...
report_path <- file.path(DIR_OUT, "Yasso15_baseline_report.txt")
sink(report_path)
cat("=========================================================\n")
