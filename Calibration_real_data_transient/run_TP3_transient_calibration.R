...
# Yasso holds its a-vector fixed. Injected here so it leaves the calibrated vector;
# the wrapper reads it from model_params["alpha_A"]. Constant = TP3_ALPHA_A_FIXED
# (Prior_specs/TP3_priors.R). alpha_S/alpha_H stay free (very-informative priors).
...
message(sprintf("flux_pair J_bar = %.3f tC/ha/yr | sigma_input window [%.3f, %.3f]",
                J_bar, TP3_INPUT_FLUX_WINDOW[1] / J_bar, TP3_INPUT_FLUX_WINDOW[2] / J_bar))
