...
t_proj <- system.time({
  projection_predictions <- do.call(rbind, lapply(seq_len(N_PP_DRAWS), function(d) {
    ...
    do.call(rbind, mclapply(plots_real, function(pid) {
      ...
      run_up    <- tryCatch(yasso20_run_engine(inputs_up, model_params, C_proj_init, clim_proj),
                            error = function(e) NULL)
      mp_eq     <- model_params; mp_eq["sigma_init"] <- 1
      xi_eq     <- tryCatch(compute_xi_mean_yasso20_engine(clim_recycle, model_params),
                            error = function(e) NULL)
      C_eq      <- if (is.null(xi_eq)) NA_real_ else
        tryCatch(sum(steady_state_yasso20_engine(mp_eq, equilibrium_lm(lm, inputs, RECYCLE_YEARS), xi_eq)),
