# =============================================================================
# run_SP1_transient_predictive.R
#
# Posterior predictive stage for SP1 transient calibration.
# Differs from run_SP1_predictive.R only in:
#   - Sources calibration_engine_transient.R and sp1_wrapper_transient.R
#   - DIR_RUNS / DIR_DIAG point to Calibration_real_data_transient/
#   - steady_state engine wrapper calls sp1_transient_init
#
# USAGE:
#   Rscript run_SP1_transient_predictive.R <RUN_ID>
# =============================================================================
...
if (length(args) >= 1) {
  ...
  rns <- list.files(
    ...
    pattern = "^SP1_posterior_[0-9]{8}_[0-9]{6}\\.rds$"
  )
  if (length(rns) == 0)
    stop("No SP1 transient posterior found in Calibration_real_data_transient/runs/")
  rns    <- sort(rns, decreasing = TRUE)
  RUN_ID <- sub("^SP1_posterior_(.+)\\.rds$", "\\1", rns[1])
...
POOL_COLS     <- "C"   # SP1 has a single carbon pool
...
posterior_phys <- readRDS(file.path(DIR_RUNS,
                                    sprintf("SP1_posterior_%s.rds", RUN_ID)))
inputs_pkg     <- readRDS(file.path(DIR_INPUTS,
                                    sprintf("SP1_inputs_%s.rds",    RUN_ID)))
...
# =============================================================================
# 2.  Parameter assembly (pass-through: all SP1 params free)
# =============================================================================
...
# TRANSIENT: sp1_transient_init replaces sp1_steady_state
...
# Initial state: terminal pool value from the repeated historical run.
# =============================================================================
...
t_proj <- system.time({
  projection_predictions <- do.call(rbind, lapply(seq_len(N_PP_DRAWS), function(d) {
    ...
    do.call(rbind, mclapply(plots_real, function(pid) {
      ...
      run_hist <- tryCatch(sp1_run_engine(inputs, model_params, C_init, xi_array),
                           error = function(e) NULL)
      ...
      # -- Initial state: terminal pool value from historical run -------------
      C_proj_init <- unlist(run_hist[nrow(run_hist), POOL_COLS])

      # -- Forward run --------------------------------------------------------
      xi_proj <- tryCatch(compute_xi_sp1_engine(clim_proj, model_params),
                          error = function(e) NULL)
      if (is.null(xi_proj)) return(NULL)

      run_proj <- tryCatch(
        sp1_run_engine(inputs_proj, model_params, C_proj_init, xi_proj),
        error = function(e) NULL)
      ...
      run_up    <- tryCatch(sp1_run_engine(inputs_up, model_params, C_proj_init, xi_proj),
                            error = function(e) NULL)
      mp_eq     <- model_params; mp_eq["sigma_init"] <- 1
      xi_eq     <- tryCatch(compute_xi_mean_sp1_engine(clim_recycle, model_params),
                            error = function(e) NULL)
      C_eq      <- if (is.null(xi_eq)) NA_real_ else
        tryCatch(sum(steady_state_sp1_engine(mp_eq, equilibrium_lm(lm, inputs, RECYCLE_YEARS), xi_eq)),
...
message("\nDone. Next: run_residual_analysis.R SP1 ", RUN_ID)
