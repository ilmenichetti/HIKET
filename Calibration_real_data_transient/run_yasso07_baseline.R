# =============================================================================
# run_yasso07_baseline.R
#
# Deterministic forward run of Yasso07 at published default parameters.
# Reference baseline for comparison with calibrated posterior predictive outputs.
#
# Initialization: steady-state (sigma_init = 1.0 has no effect on SS anyway).
#
# OUTPUTS (diagnostics/Yasso07_baseline/):
#   Yasso07_baseline_predictions.rds
#   Yasso07_baseline_residuals.csv
#   Yasso07_baseline_plot_ranking.csv
#   Yasso07_baseline_mean_soc_trajectory.png   -- absolute SOC level over time
#   Yasso07_baseline_delta_soc_trajectory.png  -- mean annual ΔSOC rate (tC/ha/yr)
#   Yasso07_baseline_obs_vs_pred.png
#   Yasso07_baseline_residuals_diagnostic.png
#   Yasso07_baseline_rf_importance.png
#   Yasso07_baseline_sampled_plots.png
#   Yasso07_baseline_worst25_timeseries.png
#   Yasso07_baseline_residuals_histogram.png
#   Yasso07_baseline_report.txt
# =============================================================================
...
MODEL_NAME <- "Yasso07"
DIR_OUT    <- "./Calibration_real_data_transient/diagnostics/Yasso07_baseline"
...
MODEL_FREE_NAMES <- setdiff(names(YASSO07_FREE_DEFAULTS), c("sigma_init","sigma_input"))
free_defaults    <- YASSO07_FREE_DEFAULTS   # sigma_init=1, sigma_input=1 by construction
...
cat(sprintf("  beta1: %.5f  beta2: %.7f  gamma: %.5f\n",
            model_params["beta1"], model_params["beta2"], model_params["gamma"]))
cat(sprintf("  delta1: %.5f  delta2: %.5f  r: %.5f\n\n",
            model_params["delta1"], model_params["delta2"], model_params["r"]))
...
predictions <- do.call(rbind, mclapply(plots_real, function(pid) {
  ...
  xi_for_ss <- tryCatch(
    compute_xi_mean_yasso07(clim[seq_len(n_ss), , drop = FALSE],
                            model_params["beta1"], model_params["beta2"], model_params["gamma"]),
    error = function(e) NULL)
...
saveRDS(predictions, file.path(DIR_OUT, "Yasso07_baseline_predictions.rds"))
...
write.csv(residuals_df, file.path(DIR_OUT, "Yasso07_baseline_residuals.csv"),
          row.names = FALSE)
...
# =============================================================================
# Per-year mean ± 1 SE of predicted total SOC across plots, with observed
# campaign means as points ± 95% CI (SE * 1.96). Shows absolute bias vs
# the two observation campaigns; complements the ΔSOC plot.
...
png(file.path(DIR_OUT, "Yasso07_baseline_mean_soc_trajectory.png"),
...
# =============================================================================
# Annual rate of SOC change since each plot's first modelled year, averaged
# across plots. Matches the form in the calibrated predictive scripts (annual
# rate in tC/ha/yr) so baseline and calibrated outputs are directly comparable.
...
png(file.path(DIR_OUT, "Yasso07_baseline_delta_soc_trajectory.png"),
    width = 12L * PX_PER_IN, height = 6L * PX_PER_IN, res = PX_PER_IN)
par(mar = c(4, 4, 3, 1))
...
png(file.path(DIR_OUT, "Yasso07_baseline_obs_vs_pred.png"),
...
png(file.path(DIR_OUT, "Yasso07_baseline_residuals_diagnostic.png"),
...
png(file.path(DIR_OUT, "Yasso07_baseline_rf_importance.png"),
...
png(file.path(DIR_OUT, "Yasso07_baseline_sampled_plots.png"),
...
write.csv(plot_resid_summary,
          file.path(DIR_OUT, "Yasso07_baseline_plot_ranking.csv"), row.names = FALSE)
...
png(file.path(DIR_OUT, "Yasso07_baseline_worst25_timeseries.png"),
...
png(file.path(DIR_OUT, "Yasso07_baseline_residuals_histogram.png"),
...
report_path <- file.path(DIR_OUT, "Yasso07_baseline_report.txt")
sink(report_path)
cat("=========================================================\n")
