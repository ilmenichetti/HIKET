# =============================================================================
# Calibration_Yasso07_production_Roihu.R
#
# PURPOSE:
#   Production calibration script for Yasso07 optimised for Roihu
#   (CSC national supercomputer, available April 2026).
#
#   Uses the HYBRID approach:
#     - Outer mclapply:  N_CHAINS chains run SIMULTANEOUSLY
#     - Inner mclapply:  each chain parallelises plot loop with CORES_PER_CHAIN
#
#   This requires Linux nested forking, which works correctly on Roihu
#   but NOT on Mac (where single-nesting should be used instead -- see
#   Calibration_Yasso07_production.R).
#
# ROIHU HARDWARE (BullSequana XH3000, AMD Turin 9965):
#   384 cores per CPU node (2 x 192-core CPUs)
#   Linux OS -- nested forking fully supported
#
# PERFORMANCE ESTIMATE (3700 plots, 50000 iterations, 4 chains):
#   Single-nesting (Mac approach on Roihu): ~8.5 hours wallclock
#   Hybrid (this script):                  ~2.2 hours wallclock
#   Speedup:                               ~4x
#
# OUTPUT STRUCTURE (shared with other model runs):
#   ./Calibration/progress_logs/   -- per-chain log files
#   ./Calibration/diagnostics/     -- R-hat, ESS, plots
#   ./Calibration/runs/            -- saved chains and posteriors
#
#   All files prefixed with "Yasso07_" to distinguish from other models.
#
# =============================================================================
...
# 0.  Configuration
#
#     This is the only section that needs to change between test and
#     production runs. Everything downstream adapts automatically.
# =============================================================================
...
# --- Plot replication (testing only) ---
# Set N_PLOTS_TEST to NA to use the real unreplicated dataset on Roihu
N_PLOTS_TEST <- 100L     # replicate to this many plots for local testing
# NA = use real data (Roihu production)

# --- MCMC settings ---
N_CHAINS <- 4L           # 4 chains: better R-hat than 3, fits Roihu allocation
N_ITER   <- 10000L       # per chain -- use 50000L for Roihu production
N_BURNIN <- 2000L        # per chain -- use 10000L for Roihu production
N_LOG    <- 200L         # log progress every N evaluations

# --- Parallelism (Roihu hybrid) ---
# Total cores used = N_CHAINS x CORES_PER_CHAIN
# Roihu has 384 cores per node -- leave some for OS and overhead
N_CHAINS        <- 4L
CORES_PER_CHAIN <- 90L   # 4 x 90 = 360 cores used, 24 left for OS

# For local testing on Mac, override these:
# N_CHAINS        <- 4L
...
message("=============================================================")
message(sprintf("  %s Calibration -- Roihu Hybrid Production Run", MODEL_NAME))
...
message(sprintf("  Cores per chain: %d", CORES_PER_CHAIN))
message(sprintf("  Total cores:     %d", N_CHAINS * CORES_PER_CHAIN))
message(sprintf("  Iterations:      %d per chain", N_ITER))
message(sprintf("  Burn-in:         %d per chain", N_BURNIN))
message(sprintf("  Plots:           %s\n",
                if (!is.na(N_PLOTS_TEST)) sprintf("%d (test replication)", N_PLOTS_TEST)
                else "real dataset (production)"))
...
if (!is.na(N_PLOTS_TEST)) {
  ...
  message(sprintf("TEST MODE: %d real plots replicated to %d.",
...
# =============================================================================
# 4.  Likelihood factory
#
#     Returns a compiled likelihood function with n_cores baked into the
#     closure. Each chain process gets its own independent instance --
#     no shared state between forked processes.
# =============================================================================

make_likelihood <- function(n_cores) {
  nc <- n_cores   # capture explicitly before cmpfun
  cmpfun(function(x) {
...
# =============================================================================
# 6.  Run MCMC chains -- HYBRID approach
#
#     Outer mclapply: N_CHAINS chains run SIMULTANEOUSLY
#       -> each chain is a separate forked process
#       -> requires Linux nested forking (works on Roihu, not Mac)
#
#     Inner mclapply (inside make_likelihood): CORES_PER_CHAIN cores
#       -> parallelises the plot loop within each chain process
#
#     Total cores active = N_CHAINS x CORES_PER_CHAIN = 4 x 90 = 360
#
#     Wallclock time = time for ONE chain (all run simultaneously)
#     vs single-nesting wallclock = N_CHAINS x per-chain time
#
#     Each chain writes to its own log file -- these are the only output
#     visible during the run since forked processes cannot print to console.
#     Monitor progress with:
#       tail -f ./Calibration/progress_logs/Yasso07_chain_*.log
# =============================================================================

message("-------------------------------------------------------------")
message(sprintf("Starting HYBRID MCMC: %d chains x %d iterations", N_CHAINS, N_ITER))
message(sprintf("  Chains run:      simultaneously (outer mclapply)"))
message(sprintf("  Cores per chain: %d (inner mclapply, plot loop)", CORES_PER_CHAIN))
message(sprintf("  Total cores:     %d", N_CHAINS * CORES_PER_CHAIN))
message(sprintf("  Wallclock est.:  ~2.2 hours (Roihu, 3700 plots, 50k iters)"))
message(sprintf("  Log files:       %s%s_chain_*.log", DIR_LOGS, MODEL_NAME))
message("  Monitor with: tail -f ./Calibration/progress_logs/Yasso07_chain_*.log")
message("-------------------------------------------------------------\n")
message("NOTE: No further console output until all chains complete.")
message("      Monitor progress via log files.\n")

set.seed(2025)
t_run <- system.time({
  
  # HYBRID: outer mclapply launches all chains simultaneously
  # Each chain forks CORES_PER_CHAIN workers for its plot loop
  chain_results <- parallel::mclapply(seq_len(N_CHAINS), function(chain_id) {
    ...
    setup <- createBayesianSetup(
      ...
      prior      = prior_yasso07,
      names      = FREE_NAMES,
      parallel   = FALSE   # inner parallelism via mclapply in likelihood
    )
  ...
  }, mc.cores = N_CHAINS)   # <-- THIS is the hybrid: all chains at once
  
})["elapsed"]

message(sprintf("All chains complete. Wallclock: %.1f minutes\n", t_run / 60))
...
for (i in seq_len(N_CHAINS)) {
  ...
  if (ok_i) {
    ...
    message(sprintf("  Chain %d: %d samples, %.0f%% finite  [%s]",
...
cat(sprintf("Run ID:    %s\n", RUN_ID))
cat(sprintf("Date:      %s\n", Sys.time()))
cat(sprintf("Approach:  Hybrid (parallel chains x parallel plots)\n"))
...
if (!is.null(gr)) {
  ...
  if (n_warn > 0) message(sprintf("WARNING: %d parameter(s) with R-hat > 1.10", n_warn))
}
...
if (!is.null(ess_vals)) {
  ...
  for (nm in FREE_NAMES) cat(sprintf("%-12s  %-10.0f\n", nm, ess_vals[nm]))
}
sink()
...
for (nm in FREE_NAMES) {
  cat(sprintf("%-12s  %8.4f  %8.4f  %8.4f  %8.4f  %8.4f\n",
              nm, post_q["2.5%",nm], post_q["25%",nm], post_q["50%",nm],
...
for (nm in c(CLIMATE_NAMES, NUISANCE_NAMES)) {
  chain_traces <- lapply(chain_results, function(r) {
    ...
    t(apply(raw, 1, to_original))[, nm]
  })
  ylim_r <- range(unlist(chain_traces), na.rm = TRUE)
  plot(chain_traces[[1]], type = "l", col = cols[1], ylim = ylim_r,
...
for (nm in c(CLIMATE_NAMES, NUISANCE_NAMES)) {
  ...
  plot(d_post, col = "steelblue", lwd = 2, xlim = xlim_r, ylim = ylim_r,
       main = sprintf("%s | %s", MODEL_NAME, nm), xlab = nm, ylab = "Density")
...
message("\n=============================================================")
message(sprintf("  %s Calibration Complete (Hybrid / Roihu)", MODEL_NAME))
message(sprintf("  Run ID:    %s", RUN_ID))
message(sprintf("  Wallclock: %.1f minutes", t_run / 60))
message(sprintf("  Approach:  %d chains x %d cores = %d total cores",
                N_CHAINS, CORES_PER_CHAIN, N_CHAINS * CORES_PER_CHAIN))
message(sprintf("  Samples:   %d post-burnin (across %d chains)",
                nrow(all_phys), N_CHAINS))
if (!is.null(gr)) {
...
if (!is.null(ess_vals)) {
  message(sprintf("  ESS:       %.0f -- %.0f  (%s)",
                  min(ess_vals), max(ess_vals),
                  if (sum(ess_vals < 200) == 0) "all > 200"
                  else sprintf("%d < 200", sum(ess_vals < 200))))
}
message("=============================================================\n")