...
# Legacy wrappers provide the cross-model YASSO*_PARAM_NAMES and
# YASSO*_DEFAULT_PARAMS constants used below (all three Yasso models).
# NOTE: these also define yasso15_run/yasso20_run WITHOUT the C_final output.
# Transient calibration scripts re-source their transient wrapper AFTER this
# file so the C_final-carrying run functions win (see run_Yasso15/20_*.R).
...
message("\nPriors_model_matching.R complete.")

# =============================================================================
# 10.  Write Prior_specs files (set WRITE_PRIORS <- TRUE before running)
# =============================================================================
# Regenerates Yasso15 and Yasso20 prior files from the FMI data loaded above.
# Yasso07 sigma_ppm is based on literature CIs (not a posterior file) and is
# not written here -- update Prior_specs/Yasso07_priors.R by hand if needed.
# SP1/TP2/TP3 are hand-set and not written here.
#
# Usage:
#   WRITE_PRIORS <- TRUE
#   source("./Model_functions_real_data/Study/Priors_model_matching.R")

if (exists("WRITE_PRIORS") && isTRUE(WRITE_PRIORS)) {
  dir.create("./Prior_specs", showWarnings = FALSE)

  # Helper: write one prior file using dput() for exact R-readable output
  .write_prior_file <- function(path, prefix, free_def, sigma_ovrd) {
    con <- file(path, "w")
    on.exit(close(con))
    cat(sprintf("# %s prior specification -- HIKET pipeline\n", prefix), file = con)
    cat(sprintf("# Generated by Priors_model_matching.R on %s\n", Sys.Date()), file = con)
    cat("# Sources: FMI Ryassofortran data files\n\n", file = con)
    cat(sprintf("%s_FREE_DEFAULTS <- ", prefix), file = con)
    dput(free_def, file = con)
    cat(sprintf("\n%s_SIGMA_PPM <- ", prefix), file = con)
    dput(sigma_ovrd, file = con)
    cat("\n", file = con)
    message("  Wrote: ", path)
  }

  FIXED_Y15 <- c("alpha_A","alpha_W","alpha_E","alpha_N",
                 "p_H","alpha_H","w1","w2","w3","w4","w5")

  # Yasso15
  y15_free     <- setdiff(YASSO15_PARAM_NAMES, FIXED_Y15)
  y15_free_def <- c(defaults15[y15_free], sigma_init = 1.0, sigma_input = 1.0)
  y15_sig_all  <- c(sigma15[y15_free],    sigma_init = 0.5, sigma_input = 0.5)
  .write_prior_file("./Prior_specs/Yasso15_priors.R", "YASSO15",
                    y15_free_def, y15_sig_all[y15_sig_all != 1.0])

  # Yasso20
  y20_free     <- setdiff(YASSO20_PARAM_NAMES, FIXED_Y15)
  y20_free_def <- c(defaults20[y20_free], sigma_init = 1.0, sigma_input = 1.0)
  y20_sig_all  <- c(sigma20[y20_free],    sigma_init = 0.5, sigma_input = 0.5)
  .write_prior_file("./Prior_specs/Yasso20_priors.R", "YASSO20",
                    y20_free_def, y20_sig_all[y20_sig_all != 1.0])

  message("Prior_specs files written.")
}