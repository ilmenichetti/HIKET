...
subtitle: "A Multi-Model Approach to Reducing Uncertainty in Climate Projections"
...
Soil organic carbon (SOC) models all share one foundational assumption — carbon pools decay at a **rate proportional to their size**.

Think of it like radioactive decay, or how a population of bacteria might decline when conditions worsen.
...
Controls how carbon flows from one pool to the next. Diagonal = losses, off-diagonal = transfers
</div>
<div class="model-card">
<h3>$\xi(T,W)$ — modifier</h3>
Scalar that rescales all rates simultaneously as a function of temperature and moisture
</div>
]
...
With three pools in a **cascade** (no back-transfers), the system expands to:
...
<p>This is how we <b>initialise</b> all models — solving analytically for equilibrium carbon stocks, avoiding costly 1000-year spin-ups.</p>
</div>
...
Different models express this relationship differently (Arrhenius functions, Q10 factors, Gaussian curves) — and this turns out to **matter a lot** for long-term projections.
</div>
...
q10_response <- exp(log(2) * temp / 10)        # Q10 = 2
...
<div>
.highlight-box[
The general theory is that carbon exists on a **quality spectrum** — some compounds are easier for microbes to break down (sugars, starch) while others are more resistant (lignin, waxes). This inherits from last centuries paradigms of humus fractionation.
Is being recently questioned by a novel interpretation, seeing carbon quality as an emergent ecosystem property. First order models still work, though, just their interpretation is changing!
]
</div>
]
...
.footnote[Both approaches use first-order kinetics — the difference is in **how many pools** and **how they're defined**.]
...
.highlight-box[
A 20–40% spread in soil carbon projections translates directly into **uncertainty in national climate commitments** — with real consequences for policy.
]
</div>
...
<br>
 *If all models were calibrated the same way, against the same data — would they still disagree?*

<br>

If yes → the disagreement reflects genuine **structural differences** in how models represent soil processes.
...
# Part 3
## How We Tackle This: Bayesian Calibration

---

## What Is Bayesian Calibration?

Most models have **parameters** — numbers that control how fast carbon decomposes, how moisture affects rates, and so on.

These parameters are usually estimated from lab experiments or previous studies. But they carry **uncertainty**.
...
Instead of one "best" answer, we work with a **distribution of plausible parameter values** — reflecting our genuine uncertainty.
</div>
...
## Why Bayesian Methods for This Problem?

.columns-3[
<div class="model-card">
<h3>🔍 Full uncertainty</h3>
We get a complete picture of parameter uncertainty — not just a best guess, but a range of plausible values with probabilities attached.
</div>

<div class="model-card">
<h3>📊 Learning from data</h3>
We can combine prior scientific knowledge (lab experiments) with field observations from Finland's ~3,700 forest plots.
</div>

<div class="model-card">
<h3>⚖️ Fair comparison</h3>
By calibrating all five models against exactly the same dataset using exactly the same method, we can make a truly fair structural comparison.
</div>
]
...
.highlight-box[
**The key insight:** We're not just fitting models to data — we're using the data to *update our beliefs* about each model's parameters, starting from the same prior information. Any remaining differences between models then reflect their **structure**, not their calibration history.
]
...
**The dataset:** ~3,700 Finnish National Forest Inventory plots with measured soil carbon, monthly climate data, and detailed litter inputs — spanning the full range of Finnish forest conditions.
...
.columns-2[
<div>
**Scenario A: Models converge after calibration**

The three Yasso generations agree with each other and with RothC/Q-model. 

→ Previous disagreements were calibration artefacts.  
→ We can confidently use any model for Finnish projections.
</div>

<div>
**Scenario B: Structural differences persist**

Even with identical calibration, models diverge under warming.

→ The conceptual choices (pool structure, temperature function) **genuinely matter**.  
→ Ensemble projections and model uncertainty must be reported.
</div>
]
...
.highlight-box[
**Either outcome is valuable.** If models agree, we reduce uncertainty. If they disagree, we quantify *how much* structural choices matter — and can target future model development where it counts most.
]
...
```
All five models are calibrated against the same ~3,700 Finnish forest plots on the **Roihu supercomputer** (384 cores), completing in days rather than the months a serial approach would require.
...
*This work is part of the HIKET project, supported by...*
